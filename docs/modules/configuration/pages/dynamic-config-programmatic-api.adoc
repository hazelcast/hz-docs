= Dynamic Configuration with Programmatic API
:description: You can use the Programmatic API to add new configuration to some supported features at runtime. Currently only Java is supported.
...
To add new configuration to a member at runtime, use the `add*Config()` method of a xref:dynamic-config-overview.adoc#supported-data-structures[supported data structure] after starting the Hazelcast instance.
