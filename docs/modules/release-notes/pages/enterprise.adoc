...
* *Improved validation for serializable Jet functions*: Added additional validation to prevent serializable functions from connecting to attacker-controlled clusters or arbitrary addresses without a valid execution context.

* *Prevented arbitrary file read through Jet*: Hardened Jet usage in serializable contexts to prevent crafted operations from reading files from members, including configuration, secrets, or data stored on disk.

* *Removed exposure from experimental declarative pipeline functionality*: Addressed a permissions issue in the Enterprise declarative pipeline component that could allow unauthorized Jet job submission and access to sources or sinks such as files, maps, Kafka, and JDBC when Jet/SQL was enabled.
...
* *Hardened Jet path and class-loading validation*: Fixed path traversal issues in Jet processor class loading and in file sinks.

* *Hardened client deserialization*: Added a default Java serialization blocklist, and extended the zero-config Compact serialization blocklist.

* *Blocked reflective access via Spring classes*: Restricted classes in Predicate API and SQL paths to prevent reflective access to Hazelcast instances and sensitive objects.

* *Hardened SQL object creation and method invocation*: Prevented unprivileged actions in IMap.

* *Rejected malformed serialized arrays*: Added validation to detect truncated array data before allocating or deserializing unexpectedly large arrays.

* *Resolved vulnerabilities in Jackson*: Addressed reported vulnerabilities, including Jackson Core async parser https://github.com/advisories/GHSA-72hv-8253-57qq[DoS GHSA-72hv-8253-57qq] and CVEs CVE-2026-54512, CVE-2026-54513, CVE-2026-54514, CVE-2026-54515, CVE-2026-54516, CVE-2026-54517, CVE-2026-54518

* *Resolved high and critical CVEs identified by security scans*: Updated affected components to resolve high and critical CVEs CVE-2026-56816, CVE-2026-2332, CVE-2026-33117, CVE-2026-41293, CVE-2026-41855, CVE-2026-42579, CVE-2026-42581, CVE-2026-42584, CVE-2026-43512, CVE-2026-43515, CVE-2026-45674, CVE-2026-47691, CVE-2026-53434, CVE-2026-55276, CVE-2026-59083, CVE-2026-59084, CVE-2025-37731, CVE-2026-24051, CVE-2026-33870, CVE-2026-33871, CVE-2026-39883, CVE-2026-41284, CVE-2026-41838, CVE-2026-41842, CVE-2026-41848, CVE-2026-41850, CVE-2026-41851, CVE-2026-42498, CVE-2026-42583, CVE-2026-42585, CVE-2026-42586, CVE-2026-42587, CVE-2026-43513, CVE-2026-44248, CVE-2026-44249, CVE-2026-44250, CVE-2026-44890, CVE-2026-44891, CVE-2026-44893, CVE-2026-45416, CVE-2026-46340, CVE-2026-48006, CVE-2026-48043, CVE-2026-48059, CVE-2026-50010, CVE-2026-50011, CVE-2026-53404, CVE-2026-54291, CVE-2026-54399, CVE-2026-54428, CVE-2026-54512, CVE-2026-54513, CVE-2026-55831, CVE-2026-55833, CVE-2026-5795

=== Fixes

* *Fixed `IndexOutOfBoundsException` in HD IMap SetOperation with `MapStore` and TTL*: Resolved an issue where a slow `MapStore` store operation could outlive an entry TTL and cause an `IndexOutOfBoundsException` while returning the old value for an HD IMap update. The fix prevents unexpected operation failures in this scenario.
...
* *Fixed Jet jobs becoming stuck after an ungraceful restart*: Resolved two issues where a running snapshot from a previous execution left the job stuck in the `COMPLETING` state, or a failed cleanup blocked subsequent snapshots.

* *Added Kubernetes state-management logging*: Added FINE and FINEST logging around Kubernetes topology state management to help diagnose clusters stuck in `NO_MIGRATION`.

* *Released retained WAN events*: Prevented the last drained event from remaining in the WAN event queue and consuming unnecessary resources.

* *Fixed persistence with `MapStore` after restart*: Prevented map operations from remaining blocked in the passive state when persistence and `MapStore` are both enabled after a cluster restart.

* *Removed redundant field deserialization*: Resolved an issue where `ReflectiveCompactSerializer` deserialized each non-primitive field twice when reading Compact-serialized objects.
...
* *Java 25 support*: Hazelcast Platform 5.7 supports Java 17, 21 and 25. Adding Java 25 enables customers to adopt the latest JDK with full compatibility, ensuring alignment with evolving JVM features and allowing teams to standardize on newer runtimes without impacting cluster stability.
+
IMPORTANT: When running Hazelcast on Java 25, the `SecurityManager` is not functional due to ecosystem changes in the JDK.
+
NOTE: Docker users who want to run Hazelcast on an earlier Java version instead of the default Java 25 must pull a JDK-tagged image. For example, `hazelcast/hazelcast:latest-jdk21` will resolve to 5.7 built with JDK 21.
+
...
* *Fixed missing OSGi package exports in Enterprise bundle*: Resolved an issue where several core Hazelcast packages were not exported correctly in the MANIFEST.MF starting from 5.5.0.
