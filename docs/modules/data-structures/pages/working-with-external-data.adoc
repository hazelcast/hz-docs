...
You can define external data stores in your `hazelcast.xml/hazelcast.yaml` configuration files or xref:configuration:configuring-programmatically.adoc[programmatically]. Defined Data Stores can be referenced in xref:integrate:jdbc-connector.adoc[JDBC Connector] and in MapStores/MapLoaders.
