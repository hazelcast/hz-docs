...
[[autostepdown]]
== Configure members to automatically step down from leadership

NOTE: This capability is only available in the {enterprise-product-name}. Your license must include ADVANCED_CP to activate this feature.
...
image:autostepdown-deployment.svg[CP Group deployment with multiple data centers]
...
IMPORTANT: Reducing the number of leadership candidates can improve stability in certain deployment scenarios, but it reduces availability. You should therefore only enable this feature if it is necessary to achieve your throughput requirements, and you should enable it on as few members as possible (up to a maximum of `(N - 1) / 2`). 

In the Raft algorithm election process, a member *must* become leader if it has the most up-to-date log in the group (https://raft.github.io/raft.pdf[In Search of an Understandable Consensus Algorithm] section 5.4.1, _Election restriction_). It is possible that the only available candidate is a member with `auto-step-down-when-leader` enabled. In this case, the `auto-step-down` member will become leader to safely transfer its up-to-date log to another member. The `auto-step-down` member rejects client operations, but they are retried internally and should succeed once the new leader is elected.
