...
User-Defined Types (also known as UDTs, nested types, or nested fields) allow you to create custom data types that can be referenced in the xref:sql:create-mapping.adoc[CREATE MAPPING statement].
...
- Type options override mapping options. Where both the mapping and type define a schema (Portable class definition, Compact schema, Java class or Avro schema) and the mapping contains only `__key` or `this` fields, the schema defined by the type is used. This means that the mapping does not need to define a schema if the type defines one.
- If the type does not specify a schema, the parent structure is used. The parent structure could be a mapping or another type.
- Type fields are optional and are resolved from the schema if not specified. Following a `CREATE MAPPING` statement, type fields are resolved and materialized. You can check the resolved fields using a `GET_DDL` query after the type has been used in a mapping. If type fields are not specified:

 ** For Portable, Compact, or Avro formats, schema fields cannot be complex (`PORTABLE`, `COMPACT`, or `RECORD`
 ). 
 ** For Java format, complex fields are allowed and map to `OBJECT`. 
...
----
<1> In `java` format, you can use the `javaClass` option to override the type class with a subclass. You must provide a concrete class for `INSERT` statements; this means that the `javaClass` option is mandatory when the original type class is abstract or interface.
<2> In `portable` format, you can use the `portableFactoryId`, `portableClassId` and `portableVersion` options to specify a portable ID for the type. This is only effective when the type is used for `__key` or `this` fields. `portableVersion` is optional. However, components cannot be inherited from the parent, so portable ID components must be defined together. Incomplete definitions have the same effect as no definition.
...
[NOTE]
====
. `EXTERNAL_NAME` aliases  are not supported for types, column names must have the same name as their corresponding Java class field or Portable/Compact/Avro schema field.
. You can mix options that belong to different formats. When you create a mapping that references your UDT, the relevant options are used and the others are ignored, which makes it possible to use a UDT in multiple mappings having different serialization formats.
. To reference another type, you must provide the column list. Otherwise, in Java format, the column will be resolved as `OBJECT` type; in Portable, Compact and Avro formats, an exception will be thrown once the type is used in a mapping.
====
...
When using the `portable` format, portable IDs (`factoryId`, `classId`, `version`) must be unique within the cluster. Otherwise, deserialization issues can occur.
...
=== Support for Cycles
Cycles between types are only supported for Java format; however, this is limited to queries. 

NOTE: The existence of referenced types are only verified when used in a `CREATE MAPPING` statement.
...
To avoid this, you can select a field-by-field instead. Additionally, this issue does not apply to Portable, Compact, or Avro types as sub-objects in these mappings and types are `GenericRecord` subclasses;
...
- `INSERT` and `UPDATE` queries are only supported for non-cyclic type hierarchies. The presence of a cycle in a type hierarchy automatically disables the ability to run these queries against any MAPPING that uses UDTs from that type hierarchy.
