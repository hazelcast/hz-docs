...
Hazelcast Set is a distributed collection that contains no duplicate elements
...
To create a Set, use the `getSet()` methods and pass it the name that you want to call your Set.
If a Set with this name already exists, Hazelcast returns that Set. Otherwise, Hazelcast creates a new Set with the given name.
...
----
<1> Create or access the Set called `set`.
<2> Add items to `set`.
--
