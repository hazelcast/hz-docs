...
NOTE: If you are running Hazelcast in embedded mode with a minimum cluster size matching the replica count, your initial deployment might fail due to dependencies on services that are still starting. The problem is caused by Kubernetes DNS only resolving headless services when the underlying Pods are ready. To avoid this problem, you can use the property `publishNotReadyAddresses` (commented out in this example). For more information, see the link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#service-v1-core[Kubernetes Service API specification].
