...
:description: To query entries in maps, you need to create a mapping with the map connector so that the SQL service knows how to access the entries in the most efficient way.
...
If values are in the JSON format, configure the `valueFormat` field as the xref:working-with-json.adoc[`JSON` or `JSON-FLAT` type].
