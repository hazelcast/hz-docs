...
This is most suitable when the endpoints have static IP addresses which will not change for the duration of the lifecycle
...
By default, the static endpoints discovery strategy will include health checks that track and periodically probe non-responsive
endpoints for liveliness. This mechanism keeps endpoints which have failed (due to connection timeouts or other exceptions)
