...
NOTE Since JDK 24, Java will give a warning when using HD Memory due to https://openjdk.org/jeps/498[JEP 498]. To avoid those warnings, please add `--sun-misc-unsafe-memory-access=allow`  to your xref:../../configuration/jvm-parameters.adoc[Java command] when launching Hazelcast.