...
- *data ingestion*: a greater number of partitions results in improved parallelism, up to around the total number of partition threads in the cluster.
  ...
  If there are fewer partitions than number of cores, not all available resources will be utilized during ingestion because updates on given partition are executed by single thread. 
...
3. For given query each vector index partition is searched by 1 thread. Number of concurrent partition searches is configured by specifying pool size for `hz:query` executor, which by default has 16 threads per member. In configurations with many cores it can be increased to fully utilize all available cores as follows:
+
[tabs] 
==== 
Java:: 
...
====
+
4. If there are fewer partitions than available cores, not all cores will be used for single search execution. This can be fine if you are focused on throughput as in general less partitions need less resources (mainly CPU). However, if you want to achieve best latency for single client, it is better to distribute the search to as many cores as possible which requires having at least as many partitions as cores in the cluster.
