...
* How many copies (or xref:data-structures:map.adoc#backing-up-maps["backups"]) of that data do you require?
* Are you going to use xref:data-structures:map.adoc#creating-sync-backups[synchronous] 
or xref:data-structures:map.adoc#creating-async-backups[asynchronous] backups?
...
NOTE: The partition count cannot be eaily changed after a cluster is created, so if you have a large cluster 
...
the cluster down. If your cluster uses xref:storage:persistence.adoc[Persistence] or xref:cp-subsystem:persistence.adoc[CP Persistence] 
...
lot of Hazelcast jobs are network-bound. A 1Gbit network connection is a 
...
Add 40% memory headroom by assuming 150GB is 60% of a cluster's total space:
