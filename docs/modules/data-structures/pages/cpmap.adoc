...
[NOTE]
====
In addition to destroying a `CPMap`, you can explicitly purge its contents using the CP Map purge API.
This is useful when you want to reclaim memory without removing the `CPMap` instance itself.
See xref:data-structures:cp-map-purge-api.adoc[CPMap Purge API,window=_blank].
====
