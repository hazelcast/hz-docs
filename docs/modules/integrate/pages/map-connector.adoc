...
To prevent data loss with `at-least-once` or `exactly-once` job guarantees when using a map journal source set to `START_FROM_CURRENT`,
...
This section describes some specific sinks related to IMap.

=== Flush IMap Sink
[.enterprise]*{enterprise-product-name}*

The `EnterpriseSinks.mapFlushSink` method forces the flush of the specified IMap to its configured MapStore when a snapshot is taken.
Unlike other sink methods, the flush does not occur on new entry processing, but only when a snapshot is triggered.
This ensures that all pending write-behind updates are persisted before the snapshot is committed or the job completes.
...
p.readFrom(Sources.map("source-imap"))
 .rebalance(Map.Entry::getKey) // optional: ensure correct partitioning
 .mapUsingServiceAsync(ServiceFactories.iMapService("target-map"),
     (map, entry) ->
         map.putAsync(entry.getKey(), entry.getValue()).toCompletableFuture())
 .writeTo(EnterpriseSinks.mapFlushSink("target-map", true));
```

[#_map_transforms]
== Map Transforms

You can use IMap to perform transformations of the items in the pipeline.
The simplest `mapUsingIMap` transformation is available in the xref:pipelines:transforms.adoc#_mapusingimap[stage API itself].

More advanced transformations are provided in `IMapExtension` ([.enterprise]*{enterprise-product-name}*):
