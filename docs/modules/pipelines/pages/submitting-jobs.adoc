...
[WARNING]
====
The following classes must be uploaded at the same time as the member starts. You cannot upload these classes using the API or the CLI.
...
To submit a job to the cluster, using SQL, use the xref:sql:create-job.adoc[`CREATE JOB` statement].
