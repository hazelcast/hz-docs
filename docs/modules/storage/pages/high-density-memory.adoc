...
To use the High-Density Memory Store, the native memory usage
...
*** With this option, memory blocks are managed using internal memory pools. Each partition thread has its own local memory pool, and pages assigned to one thread are not shared with others.
*** Memory is allocated in blocks called pages, each having a 4 MB size by default. These pages are divided into chunks or merged to create larger chunks when required.
...
*** The allocator never frees memory blocks back to the operating system.
Instead, it marks disposed memory blocks as available for reuse, so future allocations can use them again. Because of this,
memory allocation and deallocation operations (except the ones
requiring larger sizes than the page size) mostly do not interact with the
operating system.
*** When memory is needed, the allocator first tries to find a suitable free block inside its internal pools. If it cannot be found, then it interacts with the operating system to request a new page.
...
Because pages cannot be shared between threads, external fragmentation is more likely when:
...
*** Even if only a small amount of the memory is used, one partition thread may still run out of memory because it cannot barrow free blocks from others, and no free page is available from the operating system. As a result, a member can fail with NativeOutOfMemoryError even when total free memory appears large. This happens when a single thread's pool runs out of contiguous free blocks for the requested allocation size.
*** To make fragmentation and memory usage easier to observe, the following metrics are provided:
**** **Max page fragmentation**: Shows how far a memory pool is from being able to provide a full-page-sized block. It indicates the thread with the highest fragmentation level. The value ranges between 0% (no fragmentation) and 100% (high fragmentation).
**** **Imbalance memory distribution (CV)**: Measures how unevenly memory is distributed across partition threads. It is calculated as the Coefficient of Variation (CV) of memory usage among threads. A low CV indicates balanced memory usage, a high CV indicates that some threads use much more or much less memory than others, which can lead to early Native OOMEs.
**** **Total unusable free memory**: Shows the total amount of free memory that cannot currently be used for allocations because one of the threads runs out of usable memory due to insufficient free space or because its remaining memory is fragmented into blocks too small to satisfy allocation requests.
...
. Perform a rolling restart of your members. See xref:maintain-cluster:restart-cluster.adoc#rolling[].
