...
because it fails to respond, but the rest of the cluster is still running, consider the following for recovery:
...
* If the cluster is running and one or more members were ejected from the cluster because it was stuck, take a 
xref:troubleshoot:remedies-for-alerts.adoc#heap-dumps[heap dump] and xref:troubleshoot:remedies-for-alerts.adoc#thread-dumps[thread dump] of any stuck members.
// broken/replaced: take a <<heap-dumps, heap dump>> and <<thread-dumps, thread-dump>> of any stuck members.
