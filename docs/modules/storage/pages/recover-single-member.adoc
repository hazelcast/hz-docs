...
If these conditions are not met, the rejoining member stops trying to join the cluster and shuts down. The cluster does not allow that member to rejoin until it has deleted its persistence store. The cluster then tries to recover the missing member’s data from backups and repartitions the data.
