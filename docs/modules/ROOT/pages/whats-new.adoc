...
Platform 5.7 adds support for Java 25, the latest Java LTS release, with no regressions against the prior LTS baseline across Oracle JDK, OpenJDK and the other JVM vendors Hazelcast supports. Benchmarks show no performance degradation, and in some workloads a measurable improvement. Management Center 5.11 is validated for Java 25 as well. Previously supported Java versions continue to work with Platform 5.7, so adopting this release does not force a JVM upgrade.
