...
If you are not working with a external data store (e.g. database) using the MapLoader feature, `map.evict` and `map.delete` have the same effect. This feature is described in detail in the xref:external-data-stores:external-data-stores.adoc[External Data Stores] section of the documentation.
