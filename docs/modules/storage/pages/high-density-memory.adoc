...
// tag::pooled-native-list[]
*** Memory is allocated in blocks called pages, each of which is 4 MB in size by default. These pages are divided into chunks or merged to create larger chunks when required.
...
// end::pooled-native-list[]
*** Note that the POOLED allocator may cause internal fragmentation because it allocates memory in power-of-two block sizes. Requests that do not match these sizes are allocated using the next larger power-of-two block, leaving part of the allocated memory unused.
*** See xref:storage:pooled-native-memory-allocator.adoc[Pooled Native Memory] for details on pooling behavior, manager types, and fragmentation.
