...
```bash
gradle build
bin/hazelcast submit build/libs/tutorial-python-1.0-SNAPSHOT.jar
```
--
Maven:: 
+ 
...
```bash
mvn package
bin/hazelcast submit target/tutorial-python-1.0-SNAPSHOT.jar
```
--
====
...
```bash
bin/hazelcast cancel python-function
```
