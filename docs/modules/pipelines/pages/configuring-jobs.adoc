...
Jobs can be configured to use a xref:clusters:user-code-namespaces.adoc[User Code Namespace] (UCN), which provides them access
to custom resources associated with that UCN. {ucn} must be enabled and a namespace configured with the desired resources created
before it can be used in jobs. Any data structures used in these jobs should also have their UCN configured to ensure
operations for that data structure also have access to the necessary custom resources.
...
be un-deployed and re-deployed while still being able to use the same custom resources within pipelines.

WARNING: {ucn} can be dynamically updated at runtime, but it is recommended that namespaces associated with Jet jobs are not changed after
initial creation. Doing so may cause job execution inconsistency between member nodes if jobs restart during the namespace update.
...
CREATE JOB myJob
OPTIONS (
    'userCodeNamespace' = 'my_namespace',
)
----
--
====