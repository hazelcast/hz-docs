...
|none
|Options to pass to the file system such as <<configuring-authentication-for-remote-file-systems, authentication options ### link doesn't work ###>>.
|
[tabs]
==== 
Statement:: 
...
|`false`
|`false` - each member will read all files it sees in the directory

`true` - assume all members see the same files, they will split the work
|
[tabs]
==== 
Statement:: 
