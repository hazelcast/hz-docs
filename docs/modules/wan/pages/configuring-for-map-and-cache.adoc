...
NOTE: For WAN-replicated `IMap` configurations, `HigherHitsMergePolicy`, `LatestAccessMergePolicy`,
and `LatestUpdateMergePolicy` require `per-entry-stats-enabled` to be set to `true` in the map configuration.
WAN replication does not replicate map configuration, so set this option for the replicated map in every
cluster that can publish or receive entries using one of these policies.
