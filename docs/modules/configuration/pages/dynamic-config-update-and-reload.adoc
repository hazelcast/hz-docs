...
:description: You can use the update and reload REST endpoints to add dynamic configuration for some supported features at runtime.

{description}

You have two options for adding dynamic configuration via configuration update and reload:

- <<reload, Reload>>: Edit the declarative configuration file which the member has configured from and then reload.
- <<update, Update>>: Include a declarative configuration file in the request parameter.

NOTE: You can use any non-lite member to send the request. Changes will be broadcast to the entire cluster. If you use reload then modifying one member's declarative configuration file is enough.
...
+
NOTE: Keep all existing configuration the same. You can only add new configuration blocks. To see what you can add see xref:dynamic-config-overview.adoc#supported-dynamic-configuration-changes[Supported Dynamic Configuration Changes]
+
...
All will have similar result. The only difference you will observe that in the response you will see different ignored configurations.
...
The REST API endpoints are idempotent. If you add a dynamic configuration for a block that already exists, the member will ignore the request and return the change you requested in the `ignoredConfigs` field.
