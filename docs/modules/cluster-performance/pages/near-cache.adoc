...
If an entry expires at the cluster, an invalidation event is also sent for that entry to the clients subscribed. This
behavior changed as of 5.7.0. Prior to 5.7.0, expiration did not cause near cache invalidation events to be sent.
If there are a lot of expirations happening in the cluster, it may create a lot of invalidation events. Batching
the invalidation events as described below may help in such expiration scenarios. Expiry of the entry at the cluster is
different from client near cache time to live configuration. Client <<near-cache-expiration, near cache `time-to-live-seconds` configuration>>
does not cause any invalidation events at the cluster, but it is only local to that client.
