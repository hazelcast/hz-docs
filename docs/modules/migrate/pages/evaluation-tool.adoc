...
Since this tool sends/receives data over a network or to/from a store, e.g., map,
the data objects go through a (de)serialization. 

If your data objects use the following serialization interfaces, then you need to
implement and register the related object factories/serializers:
...
See the above links for information on how you can implement and register object factories. For custom serialization,
you only need to register your serializer, without any factory.

NOTE: Implementation and/or registration of factories and custom serializers should be done only on the
Hazelcast 5.0 side; there is no need to perform these on the 3.x data since it does not need to
deserialize objects.
