...
The best performance for `VectorCollection` can be achieved by enabling the https://openjdk.org/jeps/338[Vector API] https://openjdk.org/jeps/448[preview] feature by specifying the flag `--add-modules jdk.incubator.vector` in xref:configuration:jvm-parameters.adoc[JVM parameters].
The https://openjdk.org/jeps/338[Vector API] makes use of the underlying CPU's https://en.wikipedia.org/wiki/Single_instruction,_multiple_data[SIMD (Single Instruction Multiple Data)] hardware to accelerate operations over `float` instances.
For example, on https://en.wikipedia.org/wiki/Advanced_Vector_Extensions[AVX2] enabled hardware with https://openjdk.org/jeps/338[Vector API] enabled, distance functions can be 2-4 times faster than when https://openjdk.org/jeps/338[Vector API] is not enabled.

NOTE: The https://openjdk.org/jeps/338[Vector API] is still preview in Java 21, see https://openjdk.org/jeps/448[JEP 448].
