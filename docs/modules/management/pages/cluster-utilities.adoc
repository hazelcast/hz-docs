...
This check involves the verification of all user-provided hooks are loaded during a Hazelcast member startup, that is, it verifies that
the executable of your application contains all the required `META-INF/services` files.
For example, there may be cases which you add a new `DataSerializer` hook for your application,
but do not update the corresponding `/META-INF/services/com.hazelcast.DataSerializerHook` file.
