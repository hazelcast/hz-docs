...
Offloading `MapStore` and `MapLoader` operations allow these operations to work asynchronously, meaning they do not block each other.
This provides faster executions of these operations, resulting in higher throughput of the whole cluster.
For this, you can use the `offload` configuration element as shown below. It is enabled by default (`true`). You can configure the offloading for each map in your cluster.
...
NOTE: Offloading cannot be used for xref:tiered-storage:overview.adoc[Tiered Storage], and for `MapLoaderLifecycleSupport`, since it is a one time operation per member along the lifecycle of a map.
