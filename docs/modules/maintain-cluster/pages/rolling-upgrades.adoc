...
. Upgrade the member's codebase to the new version by replacing the Hazelcast JAR file in your installation directory.
...
. [[upgrading-cluster-version]]When all members have been updated, trigger a rolling upgrade on the cluster using Management Center or `hz-cluster-admin`:
+
