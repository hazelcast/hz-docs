...
. Extract it into folder and go into `./bin` folder.
...
```bash
gradle build
bin/hazelcast submit build/libs/pulsar-example-1.0-SNAPSHOT.jar
```
--
Maven:: 
+ 
...
```bash
mvn package
bin/hazelcast submit target/pulsar-example-1.0-SNAPSHOT.jar
```
--
====
...
```bash
bin/hazelcast restart pulsar-message-counter
```
...
+
```bash
bin/hazelcast cancel pulsar-message-counter
```
