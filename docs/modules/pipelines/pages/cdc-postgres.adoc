...
NOTE: At the time of writing, there was a known https://github.com/apache/camel-quarkus/issues/6911[issue] affecting Debezium PostgreSQL connector. In our internal tests, we encountered problems with the `decoderbufs plugin`. As a result, we recommend using the `pgoutput` plugin by setting `plugin.name` = `pgoutput`. If the default configuration does not work for you, consider switching to `pgoutput`, which is PostgreSQL’s built-in logical decoding plugin. Note also that we explicitly set the `publication.name` parameter to avoid conflicts when running multiple jobs concurrently, which could otherwise cause issues with publication primary keys. We strongly recommend you assign a meaningful value to this parameter when using the `pgoutput` decoding plugin.
