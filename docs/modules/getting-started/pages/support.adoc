...
* Severity 1 is only for current outages in production environments, and there is no workaround available.
* Severity 2 is for current outages in production environments, but there is a workaround available. (e.g. directing users to static content).
* Severity 3 is for issues in development, low-impact production issues, or analysis of a production issue which is no longer ongoing.
* Severity 4 is for questions, suggestions, and all other issues.
