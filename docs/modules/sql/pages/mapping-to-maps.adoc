...
If key or value is a Portable or Compact complex type, you must enumerate all required columns 
in the `CREATE MAPPING` statement with the `EXTERNAL NAME` modifier. For example, 
to create a mapping for `IMap<Employee, Integer>`:
...
OPTIONS (
    'keyFormat'='portable',
    'keyPortableFactoryId'='<employee_portable_factory_id>'
    'keyPortableClassId'='<package>.Employee',
    'keyPortableClassVersion'='<employee_class_version>'
    'valueFormat'='int'
)
```
