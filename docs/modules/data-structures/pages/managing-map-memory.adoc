...
The eviction process starts when you try
to add an entry to a map. If the entry count in the entry's partition exceeds the `partition-maximum-size`, eviction starts on that partition.
