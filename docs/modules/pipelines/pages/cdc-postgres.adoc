...
NOTE: By the time of writing this guide, there was an https://github.com/apache/camel-quarkus/issues/6911[issue in Debezium/drivers], that caused some issues when using PostgreSQL database. In our internal tests we had a problem with using `decodebuf` plugin, therefore we use `plugin.name` = `pgoutput` here and would recommend checking it if default configuration won't work for you.
