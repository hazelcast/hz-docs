...
This filter applies only to zero-config Compact serialization. Classes that have an explicitly registered serializer are not affected by the filter, even if the filter would otherwise block them.

The filter uses an allowlist and a blocklist, in the same format as the xref:serialization-configuration.adoc#untrusted-classes[Java serialization filter]. You can allow or block classes by fully qualified class name, package name, or class-name prefix. The filter can be set on both members and clients, and the configuration does not have to be the same on each.

When a filter is configured:

* If a class is on the blocklist, serializing an object of that class throws an exception.
* If a class is on the blocklist, deserializing its data returns a `GenericRecord` instead of an instance of the class, rather than throwing an exception. This means member-side code, such as an entry processor, can still access the data through the `GenericRecord`, and features that read individual fields, such as the Predicate API and SQL, can still process the data without deserializing it into an instance of the blocked class.
...
To disable zero-config Compact serialization entirely, configure an empty allowlist and set `defaults-disabled` to `true`.
...
definitions to change safely over time. A more comprehensive guide covering all schema change scenarios is available in
the xref:schema-evolution-full.adoc[Schema Evolution] page.
