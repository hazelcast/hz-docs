...
This topic is aimed at developers and discusses the pros and cons of using the {java-client} bundled in the standard Hazelcast package, or the {java-client-new}. It also describes basic usage, configuration, and advanced features.
...
Alternatively, if you want to use a standalone or lightweight Java client, you can try the {java-client-new}. This client is currently available as Beta functionality but can interact with a Hazelcast cluster without being a full member. Note that the {java-client-new} doesn't have full feature parity with the {java-client} yet and is not recommended for production environments. For more info, see xref:java#java-client-standalone-beta[].
...
If you are a Java developer, you can choose to embed Java or use the standalone client. The decision will depend on your specific use case, scalability requirements, and the features you require. The pros and cons of each are listed below.
