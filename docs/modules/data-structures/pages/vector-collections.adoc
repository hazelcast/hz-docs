...
Vector collection indexing and search are very computationally intensive and can utilize SIMD instructions.
For best vector collection perfomance use Java 21 and enable Vector API from Project Panama by adding `--add-modules jdk.incubator.vector` to xref:configuration:jvm-parameters.adoc[JVM start command].

If you use docker, you can use the following example command:

```sh
docker run --rm -p 5701:5701 -e "JAVA_OPTS=--add-modules jdk.incubator.vector -Dhazelcast.partition.count=16" -e HZ_LICENSEKEY=<license key> hazelcast/hazelcast-enterprise:5.5.0-SNAPSHOT-slim-jdk21
```
