...
* If you cannot connect from more than one processor (eg. if you are using
Mongo Atlas offering with 1 client only), you have to ensure you use `forceReadTotalParallelismOne` option. This is due
to the fact, that Hazelcast uses `$function` aggregate behind the scenes to parallelize the work and `$function` is may
not be available on some environments like Mongo Atlas.
