...
This section discusses the scenarios in which you might want to use a xref:data-structures:working-with-external-data.adoc[MapStore] to ingest data into Hazelcast.

=== Read and Write-Through Caching

If your data is stored in a data source that is too slow to query, it's best to cache that data in Hazelcast. Such a data source is usually some kind of database or other kind of key-value storage. xref:data-structures:working-with-external-data.adoc[MapLoader and MapStore] is a tool for keeping a Hazelcast cache in sync with the data source.
