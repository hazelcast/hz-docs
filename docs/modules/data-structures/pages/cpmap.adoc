...
atomic `compareAndSet`, `delete`, `get`, `put`, `remove` and `set` operations. Since `CPMap` is a 
distributed key-value implementation, these operations involve remote calls and thus their 
performance differs from non-distributed key-value data structures, e.g. `java.util.Hashmap`. 
`CPMap` is an enterprise only feature.

WARNING: There is no unsafe variant of `CPMap`, unlike other CP data structures. Therefore, CP must
be enabled before making use of `CPMap`.
...
configuration (see: `maps` within `hazelcast-full-example.xml` and respectively `hazelcast-full-example.yaml`).

The backing data structure of a `CPMap` is stored on the JVM heap. Consequently the user must ensure
that the JVM heap is appropriately sized. Currently there is a maximum `CPMap` size of `2000MB`.

WARNING: A `CPMap` is a CP data structure and therefore takes part in a process called _snapshotting_.
When defining the capacity of a `CPMap` you must consider the following memory requirements: on-heap 
capacity for the `CPMap` and on-heap capacity for a `CPMap` snapshot per-follower within the respective
CP Group. Therefore, at a minimum for a `CPMap` with a capacity of `5MB` within a 3-member CP Group,
