...
In Hazelcast, eviction is performed on a per-partition basis. When a map reaches its configured size limit, Hazelcast checks each partition individually to determine if eviction is needed. The LRU (Least Recently Used) policy is then applied within that partition: entries in the partition are first sampled(15 by default) then sorted by their last access time, and the least recently used entry is evicted.
