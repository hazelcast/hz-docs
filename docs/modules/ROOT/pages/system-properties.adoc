...
Map Store configuration is `false`. See xref:data-structures:working-with-external-data.adoc[] for the description of the `write-coalescing` element.
...
[[rebalance-delay]]
|`hazelcast.partition.rebalance.after.member.left.delay.seconds`
|0
|int
|Time in seconds to wait before triggering automatic partition rebalancing after a member leaves the cluster by means other than xref:maintain-cluster:shutdown.adoc#graceful-shutdown[graceful shutdown].
