...
`WAN` and `MEMBER` protocols, it is important to **ensure that endpoint ranges do not overlap**.
Each protocol should be configured with unique endpoints that are not used in other protocols,
otherwise WAN replication in environments where IPs are reused may cause the cluster to
become unstable.
