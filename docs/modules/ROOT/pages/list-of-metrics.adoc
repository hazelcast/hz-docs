...
alphabetical order. The metrics are collected per
member and are specific to the local member from which
...
a cluster-wide agreed value, that is, they may show the values obtained
by communicating with other members in the cluster. This type of
metric reflects the member's local view of the cluster (consider split
...
with the oldest member of the cluster.
...
|ms
|[[clusterStartTime]]Start time of the cluster (when the first member of the cluster joins, and its localClockTime value is saved as clusterStartTime)

|`cluster.clock.clusterTime`
|ms
|Elapsed time since the oldest cluster member was created (<<clusterStartTime,`cluster.clock.clusterStartTime`>>)

|`cluster.clock.clusterTimeDiff`
|ms
|Difference between the local time (`cluster.clock.localClockTime`) of the oldest member of the cluster
...
== Jet Engine Cluster-Wide Metrics
...
== Jet Engine Job-specific Metrics
