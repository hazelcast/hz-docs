...
Starting with *Hazelcast 5.7*, Hazelcast supports purging entries from CPMaps via
`CPDataStructureManagementService`.

The purge operation removes entries that are *older than a specified duration*.
It is intended for controlled cleanup of long-lived CPMaps.
...
Monitoring is recommended for large CPMaps or frequent purge usage.
