...
:description: pass:q[You can install the xref:editions.adoc[Open Source edition] of Hazelcast, using the command line interface (CLI), Docker, the binary, or Java.]
:page-aliases: installing-using-download-archives.adoc, deploy:running-in-modular-java.adoc

{description}

TIP: Looking for Hazelcast Enterprise? See xref:install-enterprise.adoc[].
...
The Java package includes both a member API and a Java client API. The member API is for xref:deploy:choosing-a-deployment-option.adoc[embedded topologies] where you want to deploy and manage a cluster in the same Java Virtual Machine (JVM) as your applications. The Java client is for connecting to an existing member in a client/server topology, such as xref:cloud:ROOT:overview.adoc[{hazelcast-cloud}].
