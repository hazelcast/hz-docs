...
// client options
Hazelcast provides a native {java-client} which you can use to connect to a Hazelcast cluster. `hazelcast.jar` is bundled in the Hazelcast standard package, so just add `hazelcast.jar` to your classpath and you can start using this client as if you are using the Hazelcast API.
...
// consider expanding the oss equivalent or reversing the logic i.e. lead with Enterprise!
...
To get started using the {java-client-new}, you need to include the `hazelcast-java-client-{full-version}.jar` dependency in your classpath. You can then start using this client as if
you are using the Hazelcast API. 

If you prefer to use Maven, add the `hazelcast-java-client` dependency to your `pom.xml`:

// check conversation about `hazelcast-enterprise-java-client` dependency and note i.e. is it as below?
// If you prefer to use Maven, add either the `hazelcast-java-client` dependency to your `pom.xml`, or the `hazelcast-enterprise-java-client` dependency (if you have an {enterprise-product-name} license).
...
To migrate from the {java-client} to the {java-client-new}, you only have to update the dependency as described above.

[java-client-standalone]
==== {java-client-new} (BETA)

The {java-client-new} is only available as Beta functionality and does not have full feature parity with the {java-client}. Please note the following differences and restrictions:

* Only programmatic configuration is enabled - you can't configure it declaratively (XML) or using system properties
* Map is the only supported data structure
...
// check latest * For smart routing only `Unisocket mode` is supported - you need to disable smart routing is supported

=== Client API
The Client API is your gateway to access your Hazelcast cluster, including distributed objects and data pipelines (jobs).

First, you must configure your client. You can configure the {java-client} xref:configuration:understanding-configuration.adoc[declaratively or
programmatically]. Currently, you can only configure {java-client-new} programmatically.
...
For further information, see <<configure-java-client, Configure the client} >>.
...
IMPORTANT: Currently, the {java-client-new} only supports the Map distributed data structure.
...
You can use a distributed Queue object with the {java-client}, as follows: 
...
The distributed data structures listed below are also supported by the {java-client}.
...
// NOTE: The {java-client-new} only supports smart routing in `Unisocket mode`. You need to disable Smart routing for clients which
// want to use temporary permissions defined in a member. 
// For more info, see xref:security:native-client-security.adoc#handling-permissions-when-a-new-member-joins[Handling permissions].
...
See the <<client-system-properties, Client System Properties section>> for a description of the `hazelcast.client.invocation.timeout.seconds` property.
...
In cases where the security established with `Config` is not enough, and
...
Fore more information, see https://docs.hazelcast.org/docs/{full-version}/javadoc/com/hazelcast/client/config/ClientSecurityConfig.html[ClientSecurityConfig API documentation].
// needs {java-client-new} Javadoc link too
...
=== Configuring CP direct-to-leader operation routing for vlients
...
provided that there is non-expired data in it. To learn how to add a near cache to your client, see <<configuring-client-near-cache, Confiure client near cache>>.
...
The following declarative `network` configuration examples include all the parent configuration elements:
...
You can also fine tune this feature for the {java-client} using the following system properties:
...
The first step initializes the `SocketInterceptor` using the defined properties.
The second step informs when the socket is connected using the `onConnect` method.
...
a factory class name or factory implementation. Fore more information, see xref:security:tls-ssl.adoc[TLS/SSL].

As explained in the TLS/SSL section, Hazelcast members use keyStores to
identify themselves (to other members) and Hazelcast clients use trustStores to
define which members they can trust. The clients also have their own keyStores, and
members have their own trustStores, so that members can
know which clients they can trust. For more information, see xref:security:tls-ssl.adoc#mutual-authentication[Mutual authentication].

// check standalone - is config different or is the content pulled from above linked page?
...
You can configure the {java-client} declaratively (XML), programmatically (API), or
using client system properties.

For declarative configuration, the {java-client} checks the following places for the client configuration file:

* **System property**: The {java-client} first checks if the `hazelcast.client.config` system property is
set to a file path e.g. `-Dhazelcast.client.config=C:/myhazelcast.xml`.
* **Classpath**: If the configuration file is not set as a system property, the {java-client} checks the classpath for the `hazelcast-client.xml` file.

If the {java-client} does not find a configuration file, it starts with the default configuration
...
// check standalone as removed from initial PR
...
* `jitter`: Specifies by how much to randomize backoff periods. Its default value is 0.
