...
+
IMPORTANT: When running Hazelcast on Java 25, the `SecurityManager` is not functional due to ecosystem changes in the JDK. Deployments that rely on `SecurityManager`-based security policies must run on Java 17 or 21.
+
