...
== Submitting a Job using Hazelcast Operator
This option is available only when the Hazelcast cluster is deployed using the Hazelcast Operator.
For more information, see link:https://docs.hazelcast.com/operator/latest/jet-job-configuration[Submit Jet jobs].

NOTE: When running Jet jobs via the Hazelcast Operator, job submission additional details can be found in the sidecar agent logs.
