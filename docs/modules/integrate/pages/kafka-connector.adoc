...
You can also explicitly specify exact initial offsets for the Kafka source using `TopicsConfig` parameter.
Note that initial offsets provided in `topicConfig` will always have a priority over offsets stored in Kafka or associated with given consumer group.
