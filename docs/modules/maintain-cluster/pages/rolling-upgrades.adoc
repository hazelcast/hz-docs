...
without a joiner. In this case, the member appoints itself as the master (oldest member) of a
...
the oldest member of the cluster. In this case, the oldest cluster member validates that the joining member's codebase version
is compatible with the current cluster version. If it is found to be compatible, then
the member joins and the oldest member of the cluster sends the cluster version, which is set on the joining
