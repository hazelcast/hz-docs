...
Applications that need to access in-memory data use `gets`, `puts`, or xref:query:overview.adoc[other queries] to access this data. However, when you work with an external data store, members of the Hazelcast cluster retrieve data from and write data to the external data store independently of the application.
...
|MapLoader
|This method gets invoked when the application requests a value from the map. If the requested value does not exist in memory, the `MapLoader.load()` method tries to load it from the data store. Once loaded into memory, the map entry remains until it is changed, moved, or evicted.
...
== Defining External Data Stores
You can define external data stores in your `hazelcast.xml/hazelcast.yaml` configuration files or xref:configuration:configuring-programmatically.adoc[programmatically]. You can then reference these data stores in the xref:integrate:jdbc-connector.adoc[JDBC Connector] and in `MapStores` and `MapLoaders`.

In the following example we define a MySQL and H2 data stores
[tabs]
====
XML::
+
...
External Data Store has the following configuration elements:

* `name` (required) - the name of this External Data Store, has to be unique, used as identifier.
* `class-name` (required) - name of the `com.hazelcast.datastore.ExternalDataStoreFactory` implementation that will be used to create instances of the data store.
* `properties` - Configuration properties for a specific implementation of the Data Store factory. Check xref:#built-in-data-store-factories[Built-in Data Store Factories] for properties supported by specific implementations
* `shared` - specifies if the factory should return reusable Data Store for different Jet jobs and `MapStores` and `MapLoaders`. Specific behaviour depends on the implementation of the factory. Default value is `true`.

You can also add new external data stores dynamically at runtime, see xref:dynamic-config.adoc[dynamic configuration].

==== Built-in Data Store Factories

* `com.hazelcast.datastore.JdbcDataStoreFactory` - creates JDBC-based External Data Stores. Implementation is based on link:https://github.com/brettwooldridge/HikariCP[HikariDataSource]. All properties are passed directly to `HikariConfig`. For available options see link:https://github.com/brettwooldridge/HikariCP#gear-configuration-knobs-baby[HikariCP configuration].
If there are more JDBC connections used on a single member from a single job, they will share the same Data Store and the connection pool.
