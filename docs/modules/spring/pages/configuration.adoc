...
== Declaring Beans using Spring Java configuration

The easiest and recommended way of configuring Spring is with Java-based configuration. Is it also very easy way to use
Spring with Hazelcast.

Example Java configuration is listed below.
...
(1) Using `@ExposeHazelcastObjects` annotation will expose all Hazelcast distributed objects (such as IMaps) as Spring beans.
(2) The HazelcastInstance bean declaration. This a standard way of declaring beans in Spring nowadays, method itself
is just a plain old Java with no "magic tricks".

Using this configuration will enable user to autowire `HazelcastInstance` and e.g. `IMap`s like in this snippet:

[source,java]
----
/// (...)
@SpringBootApplication
@Import(SpringHazelcastConfiguration.class)
public class SpringApplication {
    ...
    @Autowired
    @Qualifier(value = "testMap") // qualifier is needed as we configured 2 IMaps, so there are two beans with the same type
    private IMap<String, String> testMap;
----

NOTE: We could've use `ExposeHazelcastObjects` directly on `SpringApplication` class and move the `hazelcastInstance`
bean declaration to this class, removing `SpringHazelcastConfiguration` completely, as `@SpringBootApplication`
is also a `@Configuration` class. We've decided to split configuration and usage just because user may also want to use
e.g. application context creation directly from Java, using `AnnotationConfigApplicationContext`.

== XML-based declaration

While Java-based configuration is now widely used and recommended, Spring users sometimes still prefer to use XML-based
configuration. Using Hazelcast in such case is also easy. User can use dedicated `hazelcast` XML namespace or stick to
plain Spring `beans` namespace.
...
While configuring Hazelcast within the Spring context XML, you can also pass values through the Spring property placeholders, as shown below.
...
[WARNING]
====
If you connect via the Hazelcast client with Spring Boot to a cluster with security enabled and want to see the health of that cluster,
