...
IMPORTANT: Reducing the number of leadership candidates can improve stability in certain deployment scenarios, but it reduces availability and fault tolerance. You should therefore only enable this feature if it is necessary to achieve your throughput requirements, and you should enable it on as few members as possible (up to a maximum of `(N - 1) / 2`). 
