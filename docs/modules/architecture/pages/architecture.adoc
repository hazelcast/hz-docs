...
Hazelcast also provides replication over WAN for high availability. You can have deployments across multiple data centers using the xref:wan:wan-replication-tutorial.adoc[WAN replication] mechanism which offers protection against a data center or wider network failures.
