...
Serialization time, deserialization time, and transmission time are tightly linked: larger or more complex objects not only take longer to serialize, but also spend more time in transit and increase network congestion.
...
Serialization speed affects network efficiency. Faster serialization reduces the time data spends waiting to be transmitted, improving both network throughput and end-to-end latency.
