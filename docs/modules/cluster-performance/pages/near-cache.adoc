...
to configure it on the server. Note that Near Cache configuration is specific to the server or client itself:
...
This is only available on Hazelcast members, not on Hazelcast clients (which have no local entries).
...
Generally, whenever the state of an entry changes in the record store by updating its value or removing it,
the invalidation event is sent for that entry.

If an entry expires at the cluster, an invalidation event is also sent for that entry to subscribed clients.
If there are a lot of expirations happening in the cluster, it may create a lot of invalidation events. 
Batching the invalidation events as described below may help improve performance in such scenarios.

NOTE: This behavior changed in Hazelcast Platform version 5.7.0. Prior to 5.7.0, expiration did not cause near cache invalidation events to be sent.

Expiry of the entry at the cluster is different from client near cache time to live configuration. Client <<near-cache-expiration, near cache `time-to-live-seconds` configuration>>
is local to the client and does not cause any invalidation events at the cluster.
