...
In the Raft algorithm, a member *must* become leader if it has the most recent log in the group (https://raft.github.io/raft.pdf[In Search of an Understandable Consensus Algorithm] section 5.4.1, _Election restriction_). In this case, a member with `auto-step-down-when-leader` enabled will become leader and transfer the latest state to another member. It will then trigger the leadership rebalancing task, and will reject client operations. If the leadership rebalancing task fails, it will retry automatically. Client operations are retried internally until a new leader is elected.

If two members are equally up-to-date, they are equally likely to become leader. To avoid `auto-step-down` members becoming leader in this scenario, a voter will reject a pre-vote request from an `auto-step-down` candidate when the log term and log index are the same. This prevents the election, reducing churn.
...
|Whether this member should automatically step down if elected leader. If enabled, `cp-member-priority` must be set to zero or below. This can only be enabled on a minority of CP members in a group (for example, two out of five). Operations that would cause this limit to be exceeded are rejected.
