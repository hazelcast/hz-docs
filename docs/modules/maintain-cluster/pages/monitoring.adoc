...
[source,plain]
----
-Dhazelcast.diagnostics.stdout=<output type>
----
...
to forward the diagnostics to any output supported by the logging framework and apply additional configurations. Using the logging framework
introduces a slight overhead in comparison to using other output types but allows for greater flexibility.
