...
NOTE: When used with `IMap`, `HigherHitsMergePolicy`, `LatestAccessMergePolicy`, and `LatestUpdateMergePolicy`
require `per-entry-stats-enabled` to be set to `true` in the map configuration. This option is `false` by default.
The same requirement applies to custom `IMap` merge policies that require per-entry statistics merge types.
