...
Normally, each Hazelcast member tracks the status of other members using local failure detectors.
But they don't share their failure detection knowledge with other cluster members. In order to
execute the Bron-Kerbosch algorithm and figure out the largest set of fully-connected members, this local failure detection data is gathered from all members. When partial network
...
for more information about failure detectors.) The oldest member in the cluster gathers all this
information and executes the maximum clique algorithm. It then decides the smallest set of Hazelcast members,
if any, to evict from the cluster so that the remaining members are fully-connected to each other again.
In a healthy cluster this set is empty.

This process has two properties to configure:

* `hazelcast.partial.member.disconnection.resolution.heartbeat.count`:  When the oldest cluster member receives
a heartbeat problem report from another member, it first waits for a number of heartbeat rounds
to allow other members to report their problems, if there are any. After that, the oldest cluster member takes all reports
received so far. It then checks the reports and updates the cluster members in a way that results in the minimum number of
member evictions from the cluster that will leave the remaining cluster members free from heartbeat problems.
+
If this configuration option is set to `0`, this functionality is disabled. It is recommended that you set this configuration option to at least `3` or `5` so that the oldest cluster member waits long enough to collect heartbeat problem reports. Otherwise, the oldest cluster member can make sub-optimal decisions. The default value is `0`.
