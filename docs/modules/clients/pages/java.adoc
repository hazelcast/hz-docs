...
See the xref:xref:transactions:providing-xa-transactions.adoc[Transactions chapter] on how to use them.
...
Using the blue-green mechanism, clients can connect to another cluster automatically when they are blacklisted from their currently connected cluster. See the xref:{page-latest-supported-mc}@management-center:monitor-imdg:monitor-clients.adoc#changing-cluster-client-filtering[Hazelcast Management Center Reference Manual] for information on blacklisting the clients.
...
xref:{page-latest-supported-mc}@management-center:monitor-imdg:monitor-clients.adoc[Monitoring Clients section^]
...
See the xref:{page-latest-supported-mc}@management-center:monitor-imdg:monitor-clients.adoc[Monitoring Clients section]
...
If you have [navy]*Hazelcast Enterprise HD*, your Hazelcast Java client's Near Cache
