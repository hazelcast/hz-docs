...
If you use High-Density Memory Store Hazelcast will also use off-heap memory for
...
metadata-space-percentage property can be configured so warning logs are emitted
when a certain threshold has been exceeded. For more details about metadata see
the High-Density Memory Store xref:storage:high-density-memory.adoc[documentation].
