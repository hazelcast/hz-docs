...
To correctly size the cluster for your use case, provide answers to as many of the 
...
the map/cache size in check, but the eviction itself does not always clear the memory 
immediately. Therefore, the answer to this question gives a good insight.
...
* When you use stream processing, what is the average approximate latency for processing of a single event?
...
If you change the partition count from the default of 271,
...
The partition count cannot be easily changed after a cluster is created, so if you have a large cluster 
be sure to test and set an optimum partition count prior to deployment. 

If you need to change the partition count after a cluster is already running, 
you will need to schedule a maintenance window to entirely bring the cluster down. 
If your cluster uses the xref:storage:persistence.adoc[Persistence] or xref:cp-subsystem:persistence.adoc[CP Persistence] 
...
backup count is set equal to or higher than three. Also, no member in a Hazelcast 
cluster will store both primary data and that data's backup.
...
=== For batch processing, garbage-free aggregation is key
