...
Hazelcast is certified on VMWare vSphere 8.0.
...
The smaller the serialized form, the less memory, disk, and backup space Hazelcast requires to store and replicate data. 

Best practices:

* Optimize the serialization format for high-volume data to reduce cluster hardware costs.
...
Every serialized object that crosses the network contributes to transmission overhead. Larger objects take longer to send, and if they exceed a network buffer's maximum transmission unit (MTU), they must be split into multiple packets.

Best practices:

* Keep serialized objects small enough to fit within a typical network buffer, e.g. MTU or socket buffer.
...
TIP: For more information about serialization options, see xref:serialization:serialization.adoc[].  
...
* Plan your serialization strategy early.
