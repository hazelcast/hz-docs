...
* `service-label-name`, `service-label-value`: service label and value used to tag services that should form the Hazelcast cluster together. Can support comma separated multiple values, an example is "label-1,label-2". Note that the number of elements in` service-label-name` and `service-label-value` must be equal. 
* `pod-label-name`, `pod-label-value`: pod label and value used to tag pods that should form the Hazelcast cluster together. Can support comma separated multiple values, an example is "label-1,label-2". Note that the number of elements in` pod-label-name` and `pod-label-value` must be equal. 
