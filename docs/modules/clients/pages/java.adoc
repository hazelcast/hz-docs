...
* https://docs.hazelcast.org/hazelcast-java-client/{java-client-standalone-version}/javadoc[Hazelcast {java-client-new} API documentation]
...
If you are using `hazelcast-enterprise-java-client`, you need to add both the `hazelcast-enterprise-java-client` dependency and private hazelcast repository to your pom.xml file, as shown below:
...
After completing the client configuration, you must create a `HazelcastClient` instance that will initialize and connect to the client based on the specified configuration:
