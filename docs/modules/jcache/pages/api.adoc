...
providers chosen as seen in the xref:providers.adoc#configuring-jcache-provider[Configuring JCache Provider section] which enables the JCache API to
...
Therefore, the exact time of removal is implementation-specific.
...
used by such entries is released as well.
