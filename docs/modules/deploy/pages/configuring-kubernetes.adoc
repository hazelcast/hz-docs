...
* `service-name`: service name used to scan only Pods connected to the given service; if not specified, then all Pods in the namespace are checked
  NOTE: If you don't specify `service-name` and fall back to a `namespace` only discovery, all pods in the namespace must be Hazelcast member pods; any other kind of Pod might block member discovery process of the Hazelcast member pods. 
