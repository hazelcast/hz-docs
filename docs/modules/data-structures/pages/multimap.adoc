...
== Creating a MultiMap on the Member Side

The following code creates a MultiMap called `multimap` on the member side.
...
public class ExampleMultiMap {
    ...
    public static void main(String[] args) {
        ...
        MultiMap<String, String> multimap = hazelcastInstance.getMultiMap("multimap");
...
== Accessing a MultiMap from Clients

The following example client codes get an existing MultiMap on the cluster, put entries into it, and print them:
...
----
<1> Get the MultiMap called `multimap` from the cluster.
