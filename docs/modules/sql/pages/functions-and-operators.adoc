...
For a guide about windowed aggregations in SQL, see xref:windowed-aggregations.adoc[].
...
`TUMBLE(TABLE(IMPOSE_ORDER(data, DESCRIPTOR(timecol), lag)), DESCRIPTOR(timecol), size)`
...
|lag
|A watermark that defines which late events are dropped.
|size
