...
Dynamic configuration allows you to add configuration for some data structures at runtime, using the Java API. If you use the `Config` object to configure a member, dynamic configuration cannot be persisted. When a cluster restarts, any changes are not reapplied to the cluster.
