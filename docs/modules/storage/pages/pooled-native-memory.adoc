...
* A thread must register to the Pooling Memory Manager before it can use the TLPMM.
...
* **Total free memory in all thread pools**: Shows the total amount of free memory that cannot currently be used for allocations because one of the threads has run out of usable memory due to insufficient free space or because its remaining memory is fragmented into blocks too small to satisfy allocation requests.
* **Total allocated memory in all thread pools**: Shows the total amount of allocated memory.
* **Total allocated external memory (from malloc) in all thread pools**: Shows the total amount of externally allocated memory from malloc. External allocation happens when requested size is larger than the page size.
