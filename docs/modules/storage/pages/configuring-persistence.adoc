...
Enables automatic removal of persistence stores.

Default: `false`

When set to `true`, the member's persistence store is automatically removed when it restarts, allowing it to join
the cluster as a completely new member.
