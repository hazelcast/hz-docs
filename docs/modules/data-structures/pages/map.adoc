...
'''
NOTE: IMap data structure can also be used by https://jet.hazelcast.org/[Hazelcast^]
for Real-Time Stream Processing (by enabling the Event Journal on your map) and
Fast Batch Processing. Hazelcast uses IMap as a source (reads data from IMap) and as a sink
...
use cases for Hazelcast. See also https://jet-start.sh/docs/api/sources-sinks#imap[here^]
in the Hazelcast Programming Guide to learn how Jet uses IMap, i.e., how it can read from and write to IMap.
