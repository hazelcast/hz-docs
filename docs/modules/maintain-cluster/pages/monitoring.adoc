...
----
{"product":"Hazelcast", "version":{version}, "pid":27746, "mode":"client", "start_timestamp":1595851430741, "licensed":0}
----
...
[source,json]
----
{"product":"Hazelcast", "version":{version}, "pid":27746, "mode":"server", "start_timestamp":1595851430741, "licensed":1}
----
...
[source,json]
----
{"product":"Hazelcast", "version":{version}, "pid":27746, "mode":"embedded", "start_timestamp":1595851430741, "licensed":1}
----
...
[[enable-hcheck]]
=== Use the health check endpoint and script
...
*{enterprise-product-name} REST API*

If you are using the {enterprise-product-name} REST API, see: https://docs.hazelcast.com/hazelcast/latest/getting-started/get-started-rest-api-with-java for information on how to enable the health check endpoint. 

After enabling the {enterprise-product-name} REST API, you can reach all the endpoints — including the health check ones — without further configuration. To see the available endpoints, and learn more about how to make a request and the expected responses etc., refer to the https://docs.hazelcast.com/hazelcast/latest/maintain-cluster/rest-api-swagger#tag/Health-Controller[Swagger documentation].

*{open-source-product-name} REST API*

If you are using the {open-source-product-name} REST API, follow the steps below to enable the health check endpoint and use the health check script.
...
[[health-check-script]]
=== Use the hz-healthcheck script

The `hz-healthcheck` script comes with the Hazelcast package.
Internally, it uses the HTTP-based health check endpoint (the {open-source-product-name} REST API). 
...
=== Use the health monitor utility
...
=== Use health check on F5 BIG-IP LTM
...
An extended set of diagnostics plugins is provided for both Hazelcast members and clients.
A dedicated log file is used to write the diagnostics content, and a rolling file approach is used to
prevent this taking up too much disk space. 

You can configure diagnostic logging using the properties described below (known as _static_ configuration) or by configuring diagnostic logging via Management Center or Operator (known as _dynamic_ configuration) without having to restart the cluster.   

=== Enable dynamic diagnostic logging (BETA)

Dynamic diagnostic logging allows you to switch logging on and off at runtime without requiring cluster restarts. 

You can also turn diagnostic logging off automatically after a set period by configuring the auto turn-off feature via Management Center or Operator. 

Operator also allows you to control the diagnostic plugins. 

For information on how to enable dynamic diagnostic logging and the auto turn-off feature, see the link:https://docs.hazelcast.com/operator/latest-snapshot/configure-diagnostic-logging[Platform Operator] or xref:{page-latest-supported-mc}@management-center:clusters:diagnostic-logging.adoc[Management Center] documentation.

NOTE: To use dynamic logging, do not enable or disable diagnostic logging statically via the properties as any configuration made using this method overrides any dynamic configuration settings. For example, if you set the `hazelcast.diagnostics.enabled` property to `true`, diagnostics cannot be disabled dynamically via Management Center or Operator. Similarly, if you set the `hazelcast.diagnostics.enabled` property to `false`, diagnostics cannot be enabled dynamically via Management Center or Operator. This also applies to other properties.

=== Enable diagnostic logging statically

To enable diagnostic logging statically, specify the following properties on the member side:
...
On the Java client side, set the following properties:
