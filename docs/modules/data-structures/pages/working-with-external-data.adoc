...
To improve cluster throughput, all MapStore operations do not block partition threads.
...
one at a time in first-in-first-out order. However, MapStore operations may take a long time to connect to the external data source and complete, blocking the affected partitions. 
...
* The `init()` and `destroy()` methods of the `MapLoaderLifecycleSupport` interface cannot be offloaded. These methods are triggered only once in the whole lifecycle of a map.
* Cluster-wide map bulk loading operations cannot be offloaded. These include `loadAll()`, `loadAllKeys()`, `putAll()` and `removeAll()` methods.
* Offloading cannot be used for xref:tiered-storage:overview.adoc[tiered storage], since you cannot enable both MapStore and tiered storage for a map.

=== Blocking Partition Threads

If you want to block partition threads until MapStore operations are complete, 
you can configure the `offload` setting for the MapStore. It is enabled by default (`true`) and can be configured for each map in your cluster.
