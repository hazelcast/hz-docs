...
memory, depending on the map's xref:data-structures:map.adoc#setting-in-memory-format[in-memory format setting].
...
this setting using a xref:data-structures:map.adoc#metadata-policy[metadata policy].
