...
List type and name of the distributed objects present in the cluster.
...
- `--type` (optional): Filter based on type
- `--show-hidden` (optional): Include objects with names starting with "__"

Example:

[source,bash,subs="attributes+"]
----
hzc list-objects --type map --show-hidden
----
