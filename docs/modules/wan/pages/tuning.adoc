...
The default behaviour is for the static endpoint discovery strategy to perform health checks on endpoints.There is no additional
configuration required beyond defining the static endpoints as our default values cover most use-cases, however the following
properties can be adjusted to suit specific needs:
