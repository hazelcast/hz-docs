...
Hazelcast List is similar to Hazelcast Set - it is a distributed collection, but it
...
To create a List, use the `getList()` methods and pass it the name that you want to call your List.
If a List with this name already exists, Hazelcast returns that List. Otherwise, Hazelcast creates a new List with the given name.
...
my_list = client.get_list("cities-list") <1>
