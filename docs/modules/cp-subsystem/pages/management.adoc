...
You can only invoke this method from the first member in the Hazelcast cluster member list. Moreover, the Hazelcast
cluster must have at least the number of members that are configured in the xref:configuration.adoc#cp-member-count[`cp-member-count`] option.
