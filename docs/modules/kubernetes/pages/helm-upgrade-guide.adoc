...
Before upgrading, review the Hazelcast xref:maintain-cluster:rolling-upgrades.adoc#hazelcast-members-compatibility-guarantees[].

== Upgrade With Persistence Disabled
...
NOTE: If hotRestart (persistence) and the Management Center persistence feature are not enabled, all scenarios might not have been tested.
...
. Upgrade your installation without disabling Management Center:
+
