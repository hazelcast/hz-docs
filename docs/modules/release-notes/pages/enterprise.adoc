...
* **Security Advisory regarding Elasticsearch 7:* https://nvd.nist.gov/vuln/detail/CVE-2025-37731[CVE-2025-37731] has been identified in Elasticsearch 7. As Elasticsearch 7 is currently End of Life (EOL), no upstream fix is available from the vendor. We strongly recommend that users evaluate the security risks associated with this CVE. Please note that support for Elasticsearch 7 will be removed in a future version of Hazelcast.

=== Fixes

* **Fixed IMap mutations failing after graceful shutdown when persistence is enabled:* Resolved an issue where certain shutdown-and-migration scenarios could leave stale records in the persistence store, later crashing the GC thread and causing subsequent `IMap` updates to fail until the member was restarted. The fix ensures persisted state is cleaned up correctly during migration so `IMap` operations remain reliable.
