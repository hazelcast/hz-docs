...
:description: You can use the /config/update and /config/reload REST API endpoints to add dynamic configuration for some supported features at runtime.

You can use the `/config/update` and `/config/reload` REST API endpoints to add xref:dynamic-config.adoc[dynamic configuration] for some xref:dynamic-config.adoc#supported-dynamic-configuration-changes[supported features] at runtime:
