...
orderMap.put( new OrderKey( 21, 1 ), order );
orderMap.put( new OrderKey( 22, 1 ), order );
orderMap.put( new OrderKey( 23, 1 ), order );
----
...
public static int removeOrder( long customerId, long orderId ) throws Exception {
    ...
    orderMap.remove( new OrderKey(orderId, customerId) );
    Set<OrderKey> orders = orderMap.keySet(Predicates.equal( "customerId", customerId ));
