...
You can configure diagnostic logging statically using the properties described below or, alternatively, 
enable _dynamic_ diagnostic logging via Management Center or Operator. 

=== Dynamic diagnostic logging

Dynamic diagnostic logging allows you to switch logging on and off at runtime without having to restart the cluster. 

You can also turn diagnostic logging off automatically after a set period by configuring the auto-off feature via Management Center or Operator. 

For information on how to enable dynamic diagnostic logging, and the auto-off feature, see the link:https://docs.hazelcast.com/operator/latest/[Platform Operator] or link:https://docs.hazelcast.com/management-center/latest/[Management Center] documentation.

// check if these links work better: 
original working link - xref:{page-latest-supported-mc}@management-center:getting-started:overview.adoc[Management Center]
New link - xref:{page-latest-supported-mc}@management-center:clusters:diagnostic-logging.adoc[Management Center]

NOTE: To use dynamic logging, do not enable or disable diagnostic logging statically via the properties as any configuration made using this method will override your dynamic configuration settings. For example, if you set the `hazelcast.diagnostics.enabled` property to `true`, diagnostics cannot be disabled dynamically via Management Center or Operator. Similarly, if you set the `hazelcast.diagnostics.enabled` property to `false`, diagnostics cannot be enabled dynamically via Management Center or Operator. This also applies to other properties.

=== Enabling diagnostics logging

To enable diagnostics logging statically, specify the following properties on the member side:
