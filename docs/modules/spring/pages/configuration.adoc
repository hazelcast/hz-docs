...
You can configure this behavior using the `@ExposeHazelcastObjects` annotation. For example, to exclude IMaps from being declared as beans:

[source,java]
----
/// (...)
@SpringBootApplication
@ExposeHazelcastObjects(excludeByType = IMap.class)
public class SpringApplication {
...
All beans registered in this way have the same names as the associated data structures. If you try to manually create a bean with a conflicting name, Hazelcast will not create the new bean.
