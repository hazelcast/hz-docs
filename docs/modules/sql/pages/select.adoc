...
- The table on the opposite side of a `RIGHT JOIN` or a `LEFT JOIN` cannot be a stream.

- At the moment, Hazelcast supports only `INNER JOIN`, `LEFT JOIN` and `RIGHT JOIN`.
