...
If this `ReliableMessageListener` can deal with message loss, the `boolean isLosstolerant()` method retursn` true`. Even though the reliable topic promises to be reliable, a `MessageListener` can be too slow, and eventually the message would become unavailable.
