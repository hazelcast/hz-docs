...
Clients attempting to connect to a Hazelcast cluster deployed on OpenShift using the Helm chart and NodePort face connectivity issues when accessing the cluster outside Kubernetes. Specifically, clients operating in smart/ALL_MEMBERS mode fetch internal pod IPs, that are inaccessible externally, which leads to connection failures after the initial handshake.
 
This issue arises due to Hazelcast's default behavior of using Kubernetes DNS mode when Role-Based Access Control (RBAC) permissions are not configured, leading to internal pod IP discovery instead of external NodePort addresses. 

You can resolve the problem by enabling the required RBAC permissions. This allows the Hazelcast Kubernetes discovery plugin to retrieve external node addresses:
