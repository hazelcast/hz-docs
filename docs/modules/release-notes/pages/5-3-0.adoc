...
* Added new messages (`uploadJobMetaData` and `uploadJobMultipart`) to the binary client protocol to enable
users to execute non-Java codes for streaming data pipelines, i.e., submitting Jet jobs.
...
* Added the ability to test whether the connection with an external data store is valid.
