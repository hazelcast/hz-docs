...
The reason is that the oldest member of the cluster, which is responsible for initiating the healing, fails to discover the other cluster's oldest member, so the split-brain merge process never begins.
...
By using this REST endpoint, you can add the missing addresses of the members from the other cluster to your members. Therefore, the oldest cluster member of one cluster will find the oldest member of the other cluster.
