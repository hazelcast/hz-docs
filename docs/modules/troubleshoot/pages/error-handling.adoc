...
== Handle errors in sources and sinks
...
Error handling is therefore concerned with what happens to a job once it
encounters a problem. By default, Hazelcast fails a job that has thrown an
error, but what you do with such a failed job afterward is discussed below.
...
to restart them (once the cause of the failure has been addressed).
...
is also to restart them from the beginning and having them completely
...
In the {open-source-product-name} of Hazelcast, this scenario is limited to fixing the
input data by some external means and then simply xref:pipelines:job-management.adoc#restarting[resuming the job].

The {enterprise-product-name} version of Hazelcast has the added option of xref:pipelines:job-update.adoc[job upgrades] which allows you to:
...
moment of the failure, because the Jet engine can only produce accurate
snapshots when in a healthy state. Instead, Hazelcast simply keeps the
