...
Compared to advanced authentication methods, with the simple authentication you don't need an additional infrastructure for Hazelcast's enterprise-level authentication (LDAP server, Kerberos, etc.), nor do you need to provide custom login module implementations as in xref:jaas-authentication.adoc[JAAS-based authentication].
