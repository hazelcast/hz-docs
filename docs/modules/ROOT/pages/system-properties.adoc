...
Note that you should always shutdown Hazelcast explicitly via using the method `HazelcastInstance.shutdown()` (see xref:maintain-cluster:shutdown.adoc#graceful-shutdown).
