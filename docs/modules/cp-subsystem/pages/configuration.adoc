...
If the CP group leader is in one of the local data centers -- and there are enough local followers to achieve quorum -- the leader will not need to wait for acknowledgements from followers in the remote data center and so the increased latency will have no impact. If the leader is in the remote data center, there will be a higher latency because each quorum calculation will be limited by the slowest roundtrip time, reducing throughput. There may also be increased latency between clients and the remote data center. To avoid this scenario, you can enable <<auto-step-down-when-leader, `auto-step-down-when-leader`>> to force CP members in the remote data center to automatically transfer leadership to another member. The increased latency will then only have an impact if a remote data center member is needed to achieve quorum.

In the Raft algorithm, a member *must* become leader if it has the most recent log in the group (https://raft.github.io/raft.pdf[In Search of an Understandable Consensus Algorithm] section 5.4.1, _Election restriction_). In this case, a member with `auto-step-down-when-leader` enabled will become leader to safely transfer leadership to another member. Client operations are rejected, but they are retried internally and should succeed once a new leader is elected.

If two members are equally up-to-date, they are equally viable leadership candidates. To help avoid `auto-step-down` members becoming leader in this scenario, a voter can reject a pre-vote request from an `auto-step-down` candidate when the log term and log index are the same. This biases elections towards members that do not have `auto-step-down` configured.

TIP: You can observe leadership transitions by searching for `LEADER (TRANSFERRING_LEADERSHIP)` in the logs. See xref:maintain-cluster:logging.adoc[].

Reducing the number of leadership candidates reduces the fault tolerance of the CP Subsystem. You should therefore only enable this feature if it is necessary to achieve your throughput requirements, and you should enable it on as few members as possible (up to a maximum of `(N - 1) / 2`). This feature does not apply to the `METADATA` group, which is not sensitive to latency.
...
|Whether this member should automatically step down if elected leader. If enabled, `cp-member-priority` must be set to zero or below. This can only be enabled on a minority of CP members in a group (`(N - 1) / 2`, for example, two out of five). Configuration that exceeds this limit will cause deployment exceptions.
