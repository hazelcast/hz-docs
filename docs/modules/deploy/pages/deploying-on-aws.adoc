= Deploying Hazelcast on Amazon AWS
:description: Deploy Hazelcast clusters on Amazon AWS EC2 and AWS ECS/Fargate environments.
...
== Understanding Member Discovery Strategies
...
Note that this plugin supports xref:clusters:partition-group-configuration.adoc#zone_aware[Hazelcast Zone Aware] feature.
...
documentation: 
xref:clusters:partition-group-configuration.adoc[Partition Group Configuration].
