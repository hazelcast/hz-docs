...
the script <<using-the-hz-cluster-admin-script, `hz-cluster-admin`>>, you can also use the REST API to manage your cluster's state.
...
and <<using-the-hz-cluster-admin-script, `hz-cluster-admin`>> sections.
+
...
for more information about membership events.
...
To change your cluster's state, you can use the https://docs.hazelcast.org/docs/{full-version}/javadoc/com/hazelcast/cluster/Cluster.html[`changeClusterState()` and `shutdown()` methods] or use the xref:management:cluster-utilities.adoc#managing-cluster-and-member-states[Management Center].
