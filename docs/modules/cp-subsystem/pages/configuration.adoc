...
Use this quickstart to test CP Subsystem in development. CP Subsystem is disabled by default, with any CP data structures operating in xref:cp-subsystem:cp-subsystem.adoc#unsafe-mode[unsafe mode]. 

NOTE: Running clusters must be restarted before any configuration changes take effect.

//RJS: Would be nice to make this a complete procedure with numbered steps for config and restart. How do you restart a cluster?

To enable CP Subsystem, configure a non-zero value for the <<cp-member-count, `cp-member-count`>> option:
...
//RJS: what about the persisted structures might make restarts fail? How do you avoid this?
