...
- Releases from 5.2 to 5.4, directly to the 5.5 LTS release. For further information on additional compatibilities for direct-to-LTS rolling upgrades, see xref:migrate:lts.adoc[].
...
With the introduction of Hazelcast's first Long-term Support (LTS) release for 5.5, you can upgrade directly from 5.2 or later to 5.5. This means that, if using a supported earlier release, you do not need to upgrade to each release between your current version and the LTS release.
...
Features marked as Beta are not upgraded. This is due to changes that can occur between Beta and general availability releases. If a Beta feature has been updated or made generally available in the target release, that version of the feature is included in the upgraded feature set.
