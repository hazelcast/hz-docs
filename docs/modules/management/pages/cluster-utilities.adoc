...
|`hz`
|Starts a Hazelcast member with customizable configuration. See <<hz-cli>>.
...
|Starts a Hazelcast member with the default settings. You should use `hz start` instead where possible -- see <<hz-cli>>.
...
To use `hz-cli`, you must enable the xref:maintain-cluster:enterprise-rest-api.adoc[].
...
TIP: You should consider using the Hazelcast xref:{page-latest-supported-clc}@clc::overview.adoc[Command Line Client (CLC)], which supports more features and does not require the REST interface.
