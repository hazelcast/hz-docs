...
* A special memory manager to decide which entries to store in the memory tier or disk tier. If a map entry is not accessed for some time, it gets flushed to the disk. If a map is frequently accessed, it stays in memory.

NOTE: An entry is never available in both tiers.
