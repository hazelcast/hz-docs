...
To implement the `MapLoader` interface you need the following methods:
...
Refer to the xref:configuration:dynamic-config.adoc[Dynamic Configuration] section to find out how to add the `DataConnection` config and new `IMap` config with `MapStore` dynamically.
