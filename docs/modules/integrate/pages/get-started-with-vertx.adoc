...
AUTHORS NOTE - This is a test to see if local build is working
...
[source,java]
----
include::ROOT:example$/vertx/vertxMainVerticleStart.java[]
----

Now, test the server response:
...
[source,java]
----
include::ROOT:example$/vertx/vertxDistributedExample1.java[]
----
...
[source,java]
----
include::ROOT:example$/vertx/vertxDistributedExample2.java[]
----
...
From now on, we will start the application with `-server` parameter, which tells Vert.x to look for a cluster manager implementation.
...
[source,java]
----
include::ROOT:example$/vertx/vertxCounterExample.java[]
----
...
For more information, such as ..., see the xxx documentation. (To do)