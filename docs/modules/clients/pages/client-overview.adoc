...
Clients are available for xref:java.adoc[Java], xref:dotnet.adoc[.NET], xref:python.adoc[Python], xref:cplusplus.adoc[C++], xref:go.adoc[Go], xref:nodejs.adoc[Node.js], and xref:memcache.adoc[Memcache]
...
For information about using the REST API for simple operations, see:
