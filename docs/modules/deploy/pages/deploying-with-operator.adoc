...
If you want to access Hazelcast cluster from outside Kubernetes, you need to use the `expose-externally` feature. Here is a sample Custom Resource definition.
...
After applying it into Kubernetes, you can verify that each Hazelcast member is exposed with a separate service.
...
* `memberAccess`: the way how each Hazelcast member is accessed
