...
These values, as well as each member's xref:performance:threading-model.adoc#io-threading[I/O Thread] counts, need to be taken into consideration when determining the appropriate client connection limits for clusters.
