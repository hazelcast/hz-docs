...
If xref:security:enabling-jaas.adoc[security] is enabled, your clients may need updated permissions to upload the JAR file used by the Kafka Connect Source Connector. For details, see xref:pipelines:job-security.adoc[].
...
- Upload the JAR file of Kafka Connect Source connector as part of your job configuration. Use this approach if you intend to use the data source for a remote server or a single pipeline.
