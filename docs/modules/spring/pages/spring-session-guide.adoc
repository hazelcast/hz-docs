...
Since 4.0, the Hazelcast integration with Spring Session is owned by the Hazelcast Team.

In order to migrate your application from using Spring Session Hazelcast 3.x to new Hazelcast Spring Session 4.x, you have to:
1. Change GroupId to `com.hazelcast` and artifactId to `hazelcast-spring-session`.
2. All Hazelcast-specific classes were moved from `org.springframework.session.hazelcast` to `com.hazelcast.spring.session`;
    you have to update your code and change the packages .
