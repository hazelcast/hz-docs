...
| 20
| int
| Maximum wait time before a join operation.
This is an upper limit on the cluster's pre-join phase duration. The pre-join
phase starts when the oldest member of the cluster receives the first join request, and ends after
no new members have tried to join for `hazelcast.wait.seconds.before.join`
seconds, or after this upper limit has elapsed (whichever comes first). Once the pre-join phase ends, the cluster moves into the join phase, during which it will only admit members that have already tried joining during the pre-join phase and are still trying to. Once the join phase is complete, the cluster will again start admitting new members.
...
| 60
| int
| Interval at which the oldest member of the cluster publishes a member list.
...
|0
|int
|When the oldest member in the cluster receives a heartbeat
problem report from another member, it first waits for a number
of heartbeat rounds to allow other members
to report their problems, if there are any. This property sets the number
of these rounds.
...
| 5
| int
| Wait time before a join operation.
This waiting time establishes a pre-join phase time window for newcomer members to
make their first join requests. When `hazelcast.wait.seconds.before.join`
elapses after the last first-timer join request (i.e., where the member hasn't
made any previous join request), or the pre-join phase has lasted for
`hazelcast.max.wait.seconds.before.join` seconds, the phase ends and the oldest cluster member starts to form the cluster.
