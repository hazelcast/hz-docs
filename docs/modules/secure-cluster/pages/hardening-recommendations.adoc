...
packages which are allowed for deserialization. Configure these restrictions on both members and clients. The configuration does not have to be the same on each. Bear in mind that a class blocked on a given member or client cannot be used there.
* Hazelcast can use zero-configuration Compact serialization for eligible classes that have no explicit serializer. We recommend restricting the classes allowed to use zero-config Compact serialization by defining an allowlist of only the classes your application needs, rather than relying on the default behavior, and configuring this on both members and clients. See xref:serialization:compact-serialization.adoc#zero-config-filter[Restricting Classes for Zero-Config Compact Serialization].
If you do not want to use zero-config Compact serialization, disable it by disabling the defaults and configuring an empty allowlist.
