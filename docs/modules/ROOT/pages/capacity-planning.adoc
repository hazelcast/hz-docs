...
If you use High-Density Memory Store Hazelcast will also use off-heap
memory for internal data structures holding your data and to track allocated memory 
blocks/pages (depending on the allocator configured). Memory used for these 
purposes is referred to as "metadata" and is tracked in exported metrics. The
metadata-space-percentage property can be configured so warning logs are emitted when
a certain threshold has been exceeded. For more details about metadata see the High-Density 
Memory Store xref:storage:high-density-memory.adoc[documentation].
