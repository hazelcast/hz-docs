...
- xref:dynamic-config-update-and-reload.adoc[REST API]. As well as the `/upload` and `/reload` endpoints, you can use the dedicated REST endpoints. For details, see the <<supported-dynamic-configuration-changes, Supported Dynamic Configuration Changes>> on this page.
