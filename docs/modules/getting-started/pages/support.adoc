...
* Level 1 is only for current outages in production environments, and there is no workaround available.
* Level 2 is for current outages in production environments, but there is a workaround available. (e.g. directing users to static content).
* Level 3 is for issues in development, low-impact production issues, or analysis of a production issue which is no longer ongoing.
