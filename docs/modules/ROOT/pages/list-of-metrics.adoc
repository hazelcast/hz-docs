...
[%collapsible]
====
All job specific metrics have their `job` (name and ID of the job) and
...
Note that this metric can be retrieved only if you enable the `storeMetricsAfterJobCompletion` setting for your jobs. See xref:pipelines:configuring-jobs.adoc#storing-metrics[Storing Job Metrics] to learn how to enable it.
