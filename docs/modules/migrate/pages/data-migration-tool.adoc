...
+
If you have not added the extension JAR ,disabled the migration of Replicated map metadata, or are not using version 5.3.8 or later, the migration fails with an error about the missing API contained in the extension JAR.

*** The migration cluster attempts to migrate the selected CP data structure. 
+
If you have not added the extension JAR to the class path, or are not using version 5.3.8 or later, the migration fails with an error about the missing API contained in the extension JAR.
...
** If you want to migrate ReplicatedMap metadata, the target cluster must be running version 5.3.8 or later. 
