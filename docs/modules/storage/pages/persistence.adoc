...
If a member restarts after the the timeframe configured in the `hazelcast.partition.rebalance.after.member.left.delay.seconds` cluster property, the cluster recovers member B's data from backups and
redistribute the data among the remaining members (members A and C
in this case). If merkle trees are enabled on any maps, migrations use those to request only missing persisted data.
