...
If pipeline's total parallelism is higher than `tasks.max` value, then `tasks.max` takes precedence.

After creation of an instance of SourceConnector, Hazelcast Jet asks to provide `tasks.max` task configurations. If Kafka
SourceConnector returns less than `tasks.max` configs, the remaining processors will become no-op processors.

When SourceConnector requests a reconfiguration, Hazelast asks for new set of configs and distributes them across the cluster.
