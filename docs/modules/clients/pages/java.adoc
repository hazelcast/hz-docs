...
=== Java Cluster Routing Modes

The Java cluster routing mode specifies how the client connects to the cluster.
...
The client is updated whenever the cluster version, partition groups, or CP group leader changes.

From {enterprise-product-name} 5.5, you can use any of the following cluster routing modes:

* Legacy **Smart** and **Unisocket** client operation modes. For information on these modes and their configuration, select **5.4** from the version picker at the top of the navigation pane. Ensure that the cluster routing mode is not configured to use the configured client operation mode
...
+
The single connected member behaves as a gateway to the other members.
...
The client does not have visibility to any cluster members outside this partition group.
--
...
and the client connection to a partition group fails, connectivity is maintained by failing over to an alternative partion group. 
If the connection lost due to an offline member, there is no attempt to retry the connection before failing over to another partition group.
For further information on Java cluster routing modes, see <<java-cluster-routing-modes,Java Cluster Routing Modes>>.
...
However, neither the `MULTI-MEMBER` nor the `SINGLE_MEMBER` cluster routing modes support backup acknowledgement to the client.
...
<<java-cluster-routing-modes, `SINGLE_MEMBER` cluster routing mode>>, we recommend that you set explicit
...
You can configure the cluster routing mode to suit your requirements, as described in <<java-cluster-routing-modes,Java Cluster Routing Modes>>.

The following examples show the configuration for each cluster routing mode. 

[NOTE]
==== 
* If you are using the `smart` or `unisocket` client operation modes, select **5.4** from the version picker above the navigation pane to see the configuration information. The cluster routing mode described below must not be present in your configuration
* If your clients want to use temporary permissions defined in a member, see 
...
ClientConfig clientConfig = new ClientConfig();
ClientNetworkConfig networkConfig = clientConfig.getNetworkConfig();
networkConfig.setClusterRoutingMode(ALL_MEMBERS);
----
...
ClientConfig clientConfig = new ClientConfig();
ClientNetworkConfig networkConfig = clientConfig.getNetworkConfig();
networkConfig.setClusterRoutingMode(SINGLE_MEMBER);
----

**MULTI_MEMBER**

To connect to a subset partition grouping, which allows direct connection to the specified group and gateway connections to other members, use the `MULTI_MEMBER` cluster routing mode, which can be defined as follows.

To use the `MULTI-MEMBER` cluster routing mode, you must also define the grouping strategy to apply. For further information on configuring partition groups, see xref:clusters:partition-group-configuration.adoc[]. 
...
+
In a scenario where all group members are killed alomost simultateously, the client loses connection but reconnects when a member starts again.
...
ClientConfig clientConfig = new ClientConfig();
ClientNetworkConfig networkConfig = clientConfig.getNetworkConfig();
networkConfig.setClusterRoutingMode(MULTI_MEMBER);
networkConfig.setClusterRoutingGroupingStrategy(PARTITION_GROUPS);
----
...
For <<java-cluster-routing-modes, Java cluster routing modes>>, the behaviour is as follows:

* If set to `ALL_MEMBERS` only the operations that are not
key-based are routed to the endpoint that is returned by the `LoadBalancer`
* If set to `SINGLE_MEMBER`, `LoadBalancer` is ignored
* If set to `MULTI_MEMBER`, best efforts are made to route operations to the required member.
If this cannot be done for any reason,  operations are routed as defined in the `LoadBalancer`

NOTE: If you are using the smart or unisocket client operation modes, select 5.4 from the version picker 
above the navigation pane to see the relevant information. 
