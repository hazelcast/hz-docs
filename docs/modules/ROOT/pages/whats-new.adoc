...
With the following additional improvements, the Enterprise-only CP Subsystem is now more efficients and performant than ever before:
...
These new features and enhancements strengthen Hazelcast's use as a flexible real-time data platform capable of delivering both the low-latency performance of AP data structures (IMap) and the consistency guarantees of distributed data structures. 

For more informationm, see xref:cp-subsystem:cp-subsystem.adoc[].
