...
Object type `Collection` resolve columns to be the same as Mongo collection properties' names and types.
...
- `wallTime DATE_TIME` - wall time of the event, which means server's date and time of the database operation.
- `ts TIMESTAMP` - Timestamp of when the even occurred, either equal to wall time of the even (see above) or to current
  time on the Hazelcast node, if no event's wall time was provided.
- `clusterTime TIMESTAMP` - cluster time on which this even occurred, which means timestamp associated with Mongo oplog entry.

Recommended Mongo documentation page: https://www.mongodb.com/docs/manual/reference/change-events/create/
