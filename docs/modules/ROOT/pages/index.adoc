...
Hazelcast {enterprise-product-name} is a licensed product that extends the {open-source-product-name} offering to provide many additional xref:getting-started:editions.adoc#key-features-and-benefits[features and benefits], plus a xref:getting-started:support.adoc[Support] subscription. 

Hazelcast is implemented in Java and has clients for Java, C++, .NET, REST, Python, Go and Node.js. Hazelcast also speaks Memcached and REST protocols.
