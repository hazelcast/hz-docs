...
.7+|_job, exec, vertex, proc, procType_
...
For keyed stages, the metric shows the number of per-key state objects currently allocated.
For non-keyed stages, it reports 1 if the processor has processed at least one item, or 0 otherwise.
See https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/jet/core/processor/Processors.html#mapStatefulP(long,com.hazelcast.function.FunctionEx,com.hazelcast.function.ToLongFunctionEx,java.util.function.Supplier,com.hazelcast.jet.function.TriFunction,com.hazelcast.jet.function.TriFunction)[Processors.mapStatefulP]
https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/jet/core/processor/Processors.html#flatMapStatefulP(long,com.hazelcast.function.FunctionEx,com.hazelcast.function.ToLongFunctionEx,java.util.function.Supplier,com.hazelcast.jet.function.TriFunction,com.hazelcast.jet.function.TriFunction)[Processor.flatMapStatefulP]
...
|`raft.group.leadership.averageTimeMs`
|ms
|Average time this CP group member has held the leader role for

|`raft.group.leadership.electedCount`
|count
|Number of times this CP group member has been elected leader

|`raft.group.memberCount`
|count
|Number of members in this CP group

|`raft.group.snapshotBuild.count`
|count
|Number of local snapshots built so far by this CP group member, including failed attempts

|`raft.group.snapshotBuild.durationMs`
|ms
|Duration of the most recently completed local snapshot build for this CP group member, including failed attempts

|`raft.group.snapshotIndex`
|count
|Raft snapshot index of this CP group

|`raft.group.snapshotTransfer.count`
|count
|Number of snapshot transfers this CP group member has installed from its leader

|`raft.group.snapshotTransfer.durationMs`
|ms
|Duration of the most recently completed snapshot transfer this CP group member installed from its leader
