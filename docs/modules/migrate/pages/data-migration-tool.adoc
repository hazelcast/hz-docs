...
** Indicative timings from our testing:
+
[NOTE]
====
These timings will be affected by entry and data structure counts as follows:

* Each entry is processed individually, in parallel, to get the entry from the source and write it to the target
* Map and ReplicatedMap data structure migrations use Jet jobs, and time must be allowed for each job to complete
====
