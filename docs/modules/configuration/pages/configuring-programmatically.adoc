...
:description: You can use a configuration object (config object) to configure members and clients with any Hazelcast client or the Java member API. To create new instances of members or clients, it's enough to pass the config object to those instances.
...
Hazelcast supports
the following methods of loading the contents of a configuration file into the configuration object.
...
== Dynamic Configuration in Java

If you use Java, you can add configuration for some data structures at runtime, using the `add*Config()` methods of supported features.

See xref:dynamic-data-structure-configuration.adoc[].