...
* `METADATA`: An internal administrative group responsible for managing CP members and CP groups. This group is initialized during cluster startup when the CP Subsystem is enabled and cannot be modified.
...
Each CP group elects its own leader, which runs the https://raft.github.io/raft.pdf[Raft consensus algorithm]. All other CP members in the group then become followers.

TIP: See http://thesecretlivesofdata.com/raft/[] for an interactive explanation of Raft.
