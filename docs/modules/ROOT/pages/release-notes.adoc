...
and in BETA state. See the xref:tiered-storage.adoc[Tiered Storage documentation].
* **Persistence for Dynamic Configuration:** You can now reload the updates made in configuration dynamically (at runtime)
after a cluster restart. See the xref:persist-dynamic-conf.adoc[documentation] for Persisting Changes in Dynamic Configuration.
