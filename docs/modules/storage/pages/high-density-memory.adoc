...
In this context metadata refers to the off-heap memory used by Hazelcast
internal data structures holding the user data and for tracking allocated memory
blocks/pages when using the `POOLED` allocator detailed above. Metadata is
...
have 246MB available for storing your data. Exceeding that will result in an
`NativeOutOfMemoryError`.

=== Estimating Usage

In general member sizing should primarily come from benchmarking in a simulated
production environment and monitoring the `memory.usedMetadata` metric. A rough
metadata usage can be estimated for initial test sizing using the following
approach:

The majority of metadata usage comes from the space allocated for underlying
data structures holding the entries for `IMap` and `ICache`. This data structure
is similar to a hashmap with a buffer expanding by a factor of 2 each time the
number of entries exceeds a load factor of 0.6 on the existing buffer size. Each
slot in the buffer costs 16 bytes and the minimum buffer size is 128. We can
define the rough cost of this structure in bytes for `n` entries with the
...
then for a cluster with `p` partitions and `m` members we can estimate per
...
metadata storage costs as structural information will be stored for each entry.

Finally the cost of tracking allocated memory blocks requires further metadata. As
a rule of thumb increase the above estimate by about 25% to cover this.
