...
respect to the link:https://hazelcast.com/foundations/distributed-computing/cap-theorem/[CAP theorem].
...
- CP Subsystem Management APIs are not available.A Hazelcast member or client that interacts with the CP Subsystem through APIs.
...
own leader, which runs the link:http://thesecretlivesofdata.com/raft/[Raft consensus algorithm]. All other CP members in the group then become followers.

A CP group leader is
responsible for handling incoming requests from callers -- Hazelcast members or clients -- and replicating those requests to
follower members. To
maintain the authority of the leader and to help lagging CP group members to make
...
For example, a cluster has three CP members and three CP groups, each CP member becomes the leader for only one CP group. If one more CP group is created, then one of the CP members becomes the leader for two CP groups.

You can give CP members a priority rating to affect their chances of becoming a leader, or configure them to reject leadership if elected. See xref:configuration.adoc#configuring-leadership-priority[Configuring Leadership Priority] and xref:configuration.adoc#autostepdown[Configuring Members to Reject Leadership].
...
member is the leader for some CP groups, those CP groups run a new leader
...
have new leaders. If a failed CP member has the follower role, it
causes a very minimal disruption since leaders are still able to replicate
...
the side that contains the majority of its CP members. If a leader falls
into the minority side, its CP group elects a new leader on the other side
