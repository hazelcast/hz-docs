...
When the remaining capacity falls below this threshold, a cleanup may be triggered during put operations to remove expired events (only if a `timeToLiveSeconds` for event journal is configured).
