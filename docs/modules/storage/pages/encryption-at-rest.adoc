...
:description: Data on disk may contain sensitive information such as in the keys or values of maps. To protect your data, you can encrypt all files in your persistence store.
...
Data in the persistence store is encrypted using symmetric encryption. The
...
store_ and, in contrast to the persistence-store encryption keys, it is not persisted
...
Encryption at Rest can be enabled and configured programmatically or declaratively, using the following options.
...
The symmetric cipher to use (such as `AES/CBC/PKCS5Padding`).
...
The size of the auto-generated persistence store encryption key.

=== `persistence.encryption-at-rest.secure-store`

The secure store to use for the retrieval of master
encryption keys. See <<configuring-secure-store, Configuring a Secure Store>> for more details.

=== Configuration Examples

The following are example configuration settings for encryption at rest.
