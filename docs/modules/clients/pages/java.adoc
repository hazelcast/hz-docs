...
* It is not possible to accurately predict which member will be a CP group leader. However, if the client is explicitly set to a CP group leader, and CP group leaders are defined algorithmically at runtime, connections to the xref:cp-subsystem:cp-subsystem.adoc[CP Subsystem] are direct-to-leader, which can result in improved performance
