...
=== Client connectivity issue with NodePort access on OpenShift

Clients attempting to connect to a Hazelcast cluster deployed on OpenShift using the Helm chart and NodePort may face connectivity issues when accessing the cluster outside Kubernetes. Specifically, clients operating in smart/ALL_MEMBERS mode fetch internal pod IPs, which are inaccessible externally, can lead to connection failures after the initial handshake.
 
This issue arises due to Hazelcast's default behavior of using Kubernetes DNS mode when Role-Based Access Control (RBAC) permissions are not configured. This leads to internal pod IP discovery instead of external NodePort addresses. 

You can resolve this problem by enabling the required RBAC permissions. This allows the Hazelcast Kubernetes discovery plugin to retrieve external node addresses:
