...
The Hazelcast Command-Line Client (CLC) is a command-line tool to connect to and operate on Hazelcast Platform and xref:cloud:ROOT:overview.adoc[{hazelcast-cloud}]. You can use the CLC to execute commands through a terminal.
...
{hazelcast-cloud} SSL configuration:
...
hazelcast:
  cluster:
    name: "CLUSTER NAME"
    cloud:
      token: "HAZELCAST VIRIDIAN TOKEN"
      enabled: true
----
