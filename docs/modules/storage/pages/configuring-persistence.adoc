...
:description: You can configure clusters to persist data structures and job snapshots on disk as well as many other options such as enabling backups, encryption, and auto-recovery.
...
NOTE: Before going into production, make sure to read the <<planning, Planning>> section.
...
- [ ] How do you want your cluster to <<cluster-restart, recover from failed shutdowns>>?
