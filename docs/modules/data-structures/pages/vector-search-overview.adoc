...
3 major elements contribute to Vector Collection memory requirements:

- entry keys and values: `memory footprint = entry count * (avg key size + avg value size)`. The sizes should be calculated in serialized form, which has a small overhead on top of the data themselves.
- vectors: `memory footprint = entry count * dimensions * 4` (`4` is the size of the `float` coordinate). The formula is for a collection with 1 vector index, with multiple vector indexes each would have to be added separately. Deduplication does not affect the computation significantly unless there are many duplicated vectors. In such case the footprint can be smaller.
...
If backups are used for fault tolerance, they need to be also included in the computation. Each backup (sync and async) needs the same amount of memory as primary data. This brings us to the final formula for memory footprint:

`total memory footprint = (1 + backup count) * entry count * (avg key size + avg value size + dimensions * 4 + max degree * 8)`

In many cases, in particular with default settings and `dimensions > 640`, vector index size contributes less than 10% to the total memory usage. In these cases the formula can be simplified and expressed in terms of raw data size (vectors and metadata):

`total memory footprint = (1 + backup count) * raw data size * 1.1`

All these data have to be distributed across the cluster members.
General recommendations from xref:ROOT:capacity-planning.adoc#memory-considerations[Hazelcast capacity planning] apply also to Vector Collection. Keep enough headroom (40% of free heap memory is recommended) to accommodate member failure or shutdown and ensure that the cluster has enough resources to xref:ROOT:capacity-planning.adoc#sizing-for-failures[operate after member failures].

If all members have the same heap size, the vector collection will need cluster at least of size:

`number of members = ceil(total memory footprint / (heap size * 60%))`

If other data will be also stored in the cluster, eg. IMaps, they also have to be accounted for.

=== CPU for ingestion

Ingestion throughput scales practically linearly with number of available CPUs as long as there are enough partitions to keep all partition threads busy and default number of partition threads, equal to number of CPUs is used.

=== CPU for search

Search throughput scales practically linearly with number of threads allocated for `hz:vector-query` executor.
In the default configuration it is half of number of vCPUs, which can be increased to get the best possible search throughput
at the expense of higher overall CPU utilisation which may affect also other operations in the cluster.
