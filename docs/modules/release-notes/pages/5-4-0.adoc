...
* Moved the MongoDB connector to the extensions module, i.e., its classes and related dependencies relocated;
...
* [.enterprise]*Enterprise* Added CPMap as a minimal key-value CP data structure. See xref:data-structures:cpmap.adoc[CPMap documentation].
...
* Enhanced the warning message shown in the cases of incorrect configurations when deploying a Hazelcast client Amazon EKS.
...
* Removed the connector for Elasticsearch 6 as Elastic discontinued the support for it.
