...
If persistence is enabled for a map or cache, its configuration is also persisted, including the related User Code Namespace. If Jet persistence is enabled, the job-related namespace configuration is also persisted.
