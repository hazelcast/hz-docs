...
Pipeline API cannot cover all possible use cases but gives a few ways in which custom transformations can be added to the pipeline.
...
`stage.using` enables use of stage extension that provides additional capabilities to the pipeline
...
|xref:integrate:map-connector.adoc#_map_transforms[IMapExtension] ([.enterprise]*{enterprise-product-name}*)
