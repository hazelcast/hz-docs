...
NOTE: You cannot yet delete an index.

NOTE: Although you can create bitmap indexes with SQL, SQL queries do not currently support bitmap index scans. As a result, bitmap indexes do not improve SQL query performance.
...
- `unique-key`: The attribute to use as a unique key source for internal unique long ID assignment.
- `unique-key-transformation`: The transformation to be applied to unique keys to generate unique long IDs from them.
