...
The `Job` class has a
https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/jet/Job.html#getMetrics--[`getMetrics()`]
method which returns a
https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/jet/core/metrics/JobMetrics.html[JobMetrics]
...
https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/jet/core/metrics/JobMetrics.html[JobMetrics API docs].
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/map/LocalMapStats.html[`LocalMapStats` Javadoc^] to see all the metrics.
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/query/LocalIndexStats.html[`LocalIndexStats` Javadoc^] to see all the metrics.
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/nearcache/NearCacheStats.html[`NearCacheStats` Javadoc^] to see all the metrics.
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/multimap/LocalMultiMapStats.html[`LocalMultiMapStats` Javadoc^] to see all the metrics.
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/collection/LocalQueueStats.html[`LocalQueueStats` Javadoc^] to see all the metrics.
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/topic/LocalTopicStats.html[`LocalTopicStats` Javadoc^] to see all the metrics.

NOTE: There is a known issue with the `ReliableTopic` structure whereby if a `MessageListener` is removed, it can still be called by the internal `MessageRunner`, resulting in duplicated topic statistics. As a result, these statistics are unreliable and should be used for general insights only.
...
See the https://docs.hazelcast.org/docs/{os-version}/javadoc/com/hazelcast/executor/LocalExecutorStats.html[`LocalExecutorStats` Javadoc^] to see all the metrics.
...
Hazelcast offers an extended set of diagnostics plugins for both Hazelcast members and  Java clients.
...
NOTE: Properties over Diagnostics will be deprecated in Hazelcast 6.0. Please, refer to the new config API. If you are using properties and new configuration APIs to configure Diagnostics, the system properties will override the configuration API. We encourage you to use the new configuration API.
...
<hazelcast>
    ...
    <diagnostics enabled="true">
        ...
        <log-directory>/user/dir/</log-directory>
        <output-type>FILE</output-type>
        <max-rolled-file-size-in-mb>50</max-rolled-file-size-in-mb>
        <max-rolled-file-count>10</max-rolled-file-count>
        <include-epoch-time>true</include-epoch-time>
        <file-name-prefix>my-logs</file-name-prefix>

        <!-- Optional properties for Diagnostics Plugins, see below for more details.-->
        <plugin-properties>
...
hazelcast:
  ...
  diagnostics:
    enabled: true
    max-rolled-file-size-in-mb: 50
    max-rolled-file-count: 10
    include-epoch-time: true
    log-directory: /user/dir
    file-name-prefix: "my-logs"
    output-type: "FILE"
    # Optional properties for Diagnostics Plugins, see below for more details.
    plugin-properties:
      "hazelcast.diagnostics.storeLatency.period.seconds": '5'
    ...
----
--
...
config.getDiagnosticsConfig()
                .setEnabled(true)
                .setLogDirectory("/your/log/directory")
                .setFileNamePrefix("my-logs")
                .setOutputType(DiagnosticsOutputType.FILE)
                .setMaxRolledFileSizeInMB(50)
                .setMaxRolledFileCount(10)
...
[source,shell]
----
java -Dhz.diagnostics.enabled=true
     -Dhz.diagnostics.output-type=STDOUT     
    # Plugins are configured over properties which can be set over DiagnosticsConfig object.
...
[source,shell]
----
JAVA_OPTS="-Dhz.diagnostics.enabled=true -Dhz.diagnostics.output-type=STDOUT -Dhazelcast.diagnostics.metric.level=info -Dhazelcast.diagnostics.invocation.sample.period.seconds=30 -Dhazelcast.diagnostics.pending.invocations.period.seconds=30 -Dhazelcast.diagnostics.slowoperations.period.seconds=30 -Dhazelcast.diagnostics.storeLatency.period.seconds=60"
----
--
====

On the Java client side, diagnostics can be configured over the `Config` object or config files similar to the member side.
...
clientConfig.getDiagnosticsConfig()
                .setEnabled(true)
                .setLogDirectory("/your/log/directory")
                .setFileNamePrefix("my-logs")
                .setOutputType(DiagnosticsOutputType.FILE)
...
Diagnostics service can be configured dynamically at runtime. You can use the following ways to configure the service dynamically:
...
+
--
For more details please refer to hazelcast k8s operator documentation.
...
spec:
  ...
  diagnostics:
    outputType: FILE
    includeEpochTime: false
    maxRolledFileCount: 10
    maxRolledFileSizeInMB: 50
    fileNamePrefix: 'pod'
    pluginProperties:
      "hazelcast.diagnostics.storeLatency.period.seconds": '5'
    pvc:
...
Over an Embedded Member::
+
--
It can be configure over the `DiagnosticsConfig` object over a member. Clients cannot set diagnostics configuration over the cluster due to security restriction.
...
hz.getConfig().setDiagnosticsConfig(diagnosticsConfig);
----
--
Management Center::
+
...
You can use the following configuration to specify the location of the diagnostics log file:
...
<hazelcast>
    ...
    <diagnostics>
        <!-- Diagnostics service configurations.-->
        <log-directory>/your/log/directory</log-directory>            
    </diagnostics>
    ...
</hazelcast>
    ...
...
hazelcast:
    ...
    diagnostics:
        log-directory: /your/log/directory
    ...
----
--
...
[source,shell]
----
java -Dhz.diagnostics.log-directory=/your/log/directory
----

.JAVA_OPTS
[source,shell]
----
JAVA_OPTS="-Dhz.diagnostics.log-directory=/your/log/directory"
----
--
====
...
```
diagnostics-<host IP>_<port>-<start time as milliseconds>-<file count in rolling>-.log
```
...
<hazelcast>
    ...
    <diagnostics>
    ...
        <file-name-prefix>my-log-prefix</file-name-prefix>      
    ...
    </diagnostics>
    ...
</hazelcast>
...
[source,shell]
----
java -Dhz.diagnostics.file-name-prefix=foobar
----

.JAVA_OPTS
[source,shell]
----
JAVA_OPTS="--Dhz.diagnostics.file-name-prefix=foobar"
----
--
====
...
A rolling file approach is used to prevent creating too much data for each run of Diagnostics. Diagnostics also can be configured dynamically. When the service enabled at runtime or new config applied while the service is enabled, diagnostics service creates a new session with a timestamp belongs to that start. The timestamp is also placed in the log file names which can be used to track log files produced by the service in a specific run. By default 10 files of 50MB each are allowed
to exist. You can set the size of each file and number of files using the following configurations. If you change the config while the service is running, the previous files will not be part of the rolling file approach anymore. The new config will be applied to the new session created by the service.
...
[source,shell]
----
java -Dhz.diagnostics.max-rolled-file-size-in-mb=100
     -Dhz.diagnostics.max-rolled-file-count=5
----

.JAVA_OPTS
[source,shell]
----
JAVA_OPTS="-Dhz.diagnostics.max-rolled-file-size-in-mb=100 -Dhz.diagnostics.max-rolled-file-count=5"
----
--
====
...
<hazelcast>
    ...
        <diagnostics>
    ...
        <output-type>FILE|STDOUT|LOGGER</output-type>
    ...  
    </diagnostics>
    ...
</hazelcast>
...
hazelcast:
    ...
    diagnostics:
        output-type: "FILE"|"STDOUT"|"LOGGER"
    ...
----
--
...
[source,shell]
----
java -Dhz.diagnostics.output-type=FILE|STDOUT|LOGGER
----

.JAVA_OPTS
[source,shell]
----
JAVA_OPTS="-Dhz.diagnostics.output-type=FILE|STDOUT|LOGGER"
----
--
====
...
<hazelcast>
    ...
    <diagnostics>        
    ...
        <output-type>LOGGER</output-type>    
    ...
    </diagnostics>
    ...
</hazelcast>
...
[source,shell]
----
java -Dhz.diagnostics.output-type=LOGGER
----

.JAVA_OPTS
[source,shell]
----
JAVA_OPTS="-Dhz.diagnostics.output-type=LOGGER"
----
--
====
...
You can configure the plugins using the following ways:
[tabs] 
==== 
XML:: 
+ 
...
[source,shell]
----
java -Dhazelcast.diagnostics.pending.invocations.period.seconds=30
----

.JAVA_OPTS
[source,shell]
----
JAVA_OPTS="-Dhazelcast.diagnostics.pending.invocations.period.seconds=30"
----
--
====
...
!NOTE: This plugin is disabled by default and it cannot be enabled or disable dynamically at runtime. If you configure to enable it before starting the member, it will be enabled but Diagnostics service cannot be disable at runtime. If you configure it after starting the member, it will not be enabled.
