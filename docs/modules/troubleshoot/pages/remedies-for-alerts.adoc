...
member as possible in order to take suitable action. For this,
...
Management Center (see the Monitoring Members sections of the Management Center documentation), `jstack`, tools such as visualVM, or a script. Take multiple snapshots of thread dumps
at 3–4 second intervals.

=== Use a script to generate a Java thread dump
...
=== Use jstack to generate thread dumps
