...
configuration: This is done using the configuration element `persistence-persistence`.
...
`persistence-persistence` element. The following are the descriptions of its attribute and sub-elements:

* `enabled`: Attribute of the `persistence-persistence` element which
...
element. As it is explained in the <<persistence-persistence, introduction>>
...
* `backup-dir`: This element is included in the `persistence-persistence` and
...
`encryption-at-rest` sub-element of `persistence-persistence`. The `encryption-at-rest`
