...
3. For given query each vector index partition is searched by 1 thread. Number of concurrent partition searches is configured by specifying pool size for `hz:query` executor, which by default has 16 threads. In server with many cores it can be increased to fully utilize all available cores as follows:
...
4. If there are fewer partitions than available cores, not all cores will be used for single search execution. This can be fine if you are focused on throughput as in general less partitions need less resources (mainly CPU). However, if you want to achieve best latency for single client, it is better to distribute the search to as many cores as possible which requires having at least this many partitions as cores in the cluster.
