...
If you are interested in using a standalone or lightweight Java client, you can try the {java-client-new}. This client can interact with a Hazelcast cluster without being a full member. For more info on features and differences between the clients, see xref:java#java-client-standalone[].
...
The {java-client-new} provides full feature parity with the {java-client} in terms of functionality, but with the following restrictions:

* Client Configuration import is not supported
* XA transactions are not supported
* Only built-in sources and sinks are supported by the Pipeline API
