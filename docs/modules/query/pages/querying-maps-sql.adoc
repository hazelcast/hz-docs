...
== Adding Different Values to Map Entries

To query maps with SQL, make sure that they contain only entries with the same structure.

For example, if an existing map contains multiple objects with varying set of keys, Hazelcast creates a mapping, using a
random sample. If and a different sample could be used each time leading to
a different column list each time.

For example, let's say you have this in a JSON-valued map:
...
However, in future optimizations, we may break this support. Therefore, in this example, it's best to make sure that both entries contain the same fields.
