...
Since 5.6, all Hazelcast data structures declared in the configuration (using for example `addMapConfig` for IMap) are automatically registered as beans when using Spring Boot.

It is possible to configure behaviour of this feature or enable it on non-Boot Spring application using `@ExposeHazelcastObjects` annotation, for example to exclude IMaps from being declared as beans, you can write:
[source,java]
----
/// (...)
@SpringBootApplication
...
Beans will have the name identical to data structure's name. If you manually create bean with conflicting name, Hazelcast won't create second bean.

==== Disabling automatic bean creation autoconfiguration
To disable automatic bean creation, please use `exclude` property of `@SpringBootApplication` annotation:
[source,java]
----
/// (...)
