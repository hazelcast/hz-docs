...
Input values in interceptors are mutable, which means that if you modify the input value in one interceptor, the next interceptor in the chain will receive the updated value. This will also affect any subsequent processing. It's therefore best to avoid modifying the input value directly. The mutability of interceptor input values is preserved for performance reasons.
