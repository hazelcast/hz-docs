...
You can query streams like you would for any batch source. The main difference between streaming and batch queries is that when you query a streaming source, Hazelcast creates a job in that background, which continues to run until you cancel it or the client disconnects.
