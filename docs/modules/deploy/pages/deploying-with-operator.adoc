...
WARNING: The `persistence` and `hazelcastClusters` parameters should be set only at the creation of the Custom Resource. Any change to the `persistence` parameter will not affect the Management Center and any change to `hazelcastClusters` will force the Management Center pod to be restarted.
