...
+
[%hardbreaks]
WARNING: Jet job's classloaders are destroyed after the job is completed. Please have in mind, that asynchronous cleanup and finalizers do not block job completion as other thread(s) would be executing those finalization/cleanup steps.
If there is some asynchronous cleanup in attached classes or some class use finalization mechanism, it may try to load some classes after the classloaders are destroyed, resulting in Class Loader leak.
If you need to use any code (library or own code), that uses async cleanup or finalizers, please consider using xref:clusters:user-code-namespaces.adoc[User Code Namespaces] to upload job resources.
