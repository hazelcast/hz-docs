...
NOTE: If you are moving members from a {open-source-product-name} cluster to an {enterprise-product-name} cluster, use an in-place rolling restart instead of a rolling upgrade. See xref:migrate:community-to-enterprise.adoc[].
...
* To use Hazelcast Management Center to trigger your rolling upgrade, upgrade Management Center before upgrading your members. For information on upgrading Management Center, refer to the xref:{page-latest-supported-mc}@management-center:getting-started:install.adoc[Management Center documentation, window=_blank], or if you deployed Management Center using the Hazelcast Operator, refer to the link:https://docs.hazelcast.com/operator/latest/deploy-management-center[Operator documentation, window=_blank].
...
* You will need the JAR file for your new version. You can download the JAR file from link:https://hazelcast.com/get-started/download/[the Hazelcast website], or update the dependency version during the upgrade procedure if you use Maven or similar.

* To use Management Center to set the cluster version, you need to upgrade Management Center before upgrading the members. See xref:{page-latest-supported-mc}@management-center:getting-started:overview.adoc#hazelcast-cluster-compatibility[Hazelcast Cluster Compatibility] for the list of versions that Management Center can set the cluster version to. Alternatively, you can use the xref:management:cluster-utilities.adoc#using-the-hz-cluster-admin-script[hz-cluster-admin] script.
+
...
. Upgrade the member's codebase to the new version. You can do this by replacing the Hazelcast JAR file in your installation directory or by updating the dependency version using Maven or similar.
