= CREATE MAPPING
:description: pass:q[The CREATE MAPPING statement creates a table that stores essential metadata about a source's data model, data connection, and serialization formats so that the SQL service can connect to the data source and query it.]
...
[EXTERNAL NAME resource_name]
[ ( column_name column_type [EXTERNAL NAME external_column_name] [, ...] ) ]
[DATA CONNECTION data_connection_name] [OBJECT TYPE resource_type]
...
You must provide the following:`mapping_name`
- `type_identifier`
...
|`resource_name`
|An optional name that identifies the resource in the external system. For example, for a Kafka connector, the identifier is the topic name. For a JDBC connector, the identifier may be the name of a table, view, or other data structure. By default, this parameter is equal to the mapping name.
|-
