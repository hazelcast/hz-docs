...
To improve cluster throughput, MapStore and MapLoader API calls are offloaded. This way partition threads are not blocked by them.
...
If you want to block partition threads until the MapStore and MapLoader API calls are complete, set the `offload` element to `false`. It is enabled by default (`true`) and can be configured for each map in your cluster.
...
mapStoreConfig.setOffload(false);
...
----
====
