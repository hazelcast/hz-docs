...
NOTE: If you are moving members from a 5.5 {open-source-product-name} cluster to an {enterprise-product-name} cluster, use an in-place rolling restart instead of a rolling upgrade. See xref:migrate:community-to-enterprise.adoc[].
...
* You will need to the JAR file for your new version. You can download the JAR file from link:https://hazelcast.com/get-started/download/[the Hazelcast website] or update the dependency version during the upgrade procedure if you use Maven
...
IMPORTANT: If you deployed your cluster using the Hazelcast Operator, follow the steps in the link:https://docs.hazelcast.com/operator/latest/scaling-upgrading#upgrading[Operator documentation] instead.
