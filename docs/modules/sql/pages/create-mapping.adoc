...
{TYPE type_identifier | DATA CONNECTION data_connection_name [OBJECT TYPE resource-type]}
...
|`external_name`
|An optional name that identifies the object or resource in the external system. For example, for a Kafka connector, the identifier is the topic name. For a JDBC connector, the identifier may be the name of a table, view, or other data structure. By default, this parameter is equal to the mapping name.
|-
...
|An optional name of a column, which is useful when loading data from an external system. If omitted, Hazelcast assumes this name is equal to the `column_name` parameter. For key-value connectors (Kafka, IMap), the key's and value's fields are prefixed by `__key` and `this`.
|
...
[source,sql]
----
CREATE MAPPING mytransactions
DATA CONNECTION mydbconnection
OPTIONS (
    'keyFormat'='varchar',
    'valueFormat'='varchar');
----
...
|Connector parameters. Check the xref:integrate:connectors.adoc[reference page] for your connector to find out what values you can use. These parameters must be enclosed in single quotes (a SQL `VARCHAR` literal).
|
```sql
