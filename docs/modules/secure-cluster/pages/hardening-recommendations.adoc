...
Similarly, the Hot Restart Store does not encrypt data. Finally, encryption passwords or
...
or non-secure hosts.
** If clients need to access the the cluster from an untrusted network, use the xref:clusters:advanced-network-configuration.adoc[advanced networking]
