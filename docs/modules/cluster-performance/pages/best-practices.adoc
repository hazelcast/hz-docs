= Best Practices
:page-aliases: performance:back-pressure.adoc, performance:best-practices.adoc, performance:caching-deserialized-values.adoc, performance:cpu-thread-affinity.adoc, performance:data-affinity.adoc, performance:near-cache.adoc, performance:performance.adoc, performance:pipelining.adoc, performance:running-on-ec2.adoc, performance:slowoperationdetector.adoc, performance:threading-model.adoc
...
One of the most effective optimization strategies for Hazelcast is to
ensure that Hazelcast services are allocated their own dedicated machine resources.
Using dedicated, properly sized hardware (or virtual hardware) ensures that
Hazelcast members have ample CPU, memory, and network resources without competing with other processes or services.

Hazelcast distributes load evenly across all its members and assumes
that the resources available to each of its members are homogeneous.
In a cluster with a mix of more and less powerful machines, the weaker
members cause bottlenecks, leaving the stronger members underutilized.
For predictable performance, it is best to use equivalent hardware for all Hazelcast members.
...
If you are using xref:serialization:implementing-portable-serialization.adoc[Portable] / xref:serialization:compact-serialization.adoc[Compact(BETA)] serialization or your map's in-memory format is `OBJECT` or `NATIVE`, then `cache-deserialized-values` element does not have any effect.
