...
:description: This quickstart will get you started with Hazelcast Platform {enterprise-product-name} and take you from installation all the way through to a working three member cluster with persistence enabled.
:page-enterprise: true
:page-aliases: enterprise-overview.adoc, persistence.adoc
...
. To start the first member in a cluster called `hello-world`, run the following Docker command:
+
...
----
<1> You can replace `docker.host.internal` with the IP address of your Docker host. Either method will work but you will need to know the IP address for later step connecting, for example, a SQL client to a specific member.
...
// more info / verification of this section
...
. In a *new* terminal window, replace `MEMBER_IP` with the IP address of one of your Hazelcast members and use the following code to start the SQL shell:
+
[source,shell]
----
...
.. Enter the IP addresses and ports of the three members in *Member Addresses*, using commas to separate entries.
// need to check - example image shows same IP but different ports yet instructions are separate IP and ports
...
* xref:getting-started:deploy-blue-green-tutorial.adoc[Set ip blue/green deployment]
