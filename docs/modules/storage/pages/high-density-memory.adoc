...
* **metadata space percentage:** Defines the percentage of the allocated native memory that is estimated to be used 
for internal High-Density Memory data structures. It is used only by the **POOLED** memory allocator. 
Its default value is **12.5**. Hazelcast will allow the actual metadata usage to exceed this defined threshold, 
if it does warning logs will be issued. For more detail see the dedicated metadata section in this doc.
...
In this context metadata refers to the off-heap memory used by Hazelcast internal data structures holding the user data and for tracking allocated memory blocks/pages when using the `POOLED` allocator detailed above. Metadata is included when calculating the total committed off-heap memory.
...
While metadata usage expands as needed this is not the case for user data. If you have 256MB native memory configured and 10MB of that is metadata, then you have 246MB available for storing your data. Exceeding that will result in an `NativeOutOfMemoryError`.

=== Estimating Usage

In general member sizing should primarily come from benchmarking in a simulated production environment and monitoring the `memory.usedMetadata` metric. A rough metadata usage can be estimated for initial test sizing using the following approach:

The majority of metadata usage comes from the space allocated for underlying data structures holding the entries for `IMap` and `ICache`. This data structure is similar to a hashmap with
a buffer expanding by a factor of 2 each time the number of entries exceeds a load factor of 0.6 on the existing buffer size. Each slot in the buffer costs 16 bytes and the minimum buffer size is 
128. We can define the rough cost of this structure in bytes for `n` entries with the following python snippet:
...
then for a cluster with `p` partitions and `m` members we can estimate per member metadata usage for an `IMap` or `ICache` with `b` backups and `n` entries to be:

```python
(p * (b + 1) * mapCost(int(n / p))) / m
```

Using the `HazelcastJsonValue` type in an `IMap` will roughly double the metadata storage costs as structural information will be stored for each entry.

Finally the cost of tracking allocated memory blocks is comparatively small. As a rule of thumb increase the above estimate by about 10% to cover this.
