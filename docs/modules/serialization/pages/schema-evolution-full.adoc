...
| All schema changes occur within the same IMap and `typeName`. This approach allows members to coexist using different schema versions during rolling upgrades. It is suitable for additive or compatible changes (for example, adding new fields).
...
StreamStage<Map.Entry<Long, OrderV1>> journal = tail
        .readFrom(Sources.<Long, OrderV1>mapJournal(hz.getMap("orders"), JournalInitialPosition.START_FROM_CURRENT))
        .withIngestionTimestamps();

// upserts
journal.filter((PredicateEx<Map.Entry<Long, OrderV1>>) e -> e.getValue() != null)
       .map(e -> Map.entry(e.getKey(), OrderV2.fromV1(e.getValue(), "JTA")))
       .writeTo(Sinks.map(v2Map));

// deletes
journal.filter(new ValueIsNullFilter())
       .writeTo(Sinks.fromProcessor("v2-remove-sink", MapRemoveP.metaSupplier("orders_v2")));

JobConfig cfg = new JobConfig()
        .setName("tail-v1-to-v2")
...
bulk.readFrom(Sources.<Long, OrderV1>map("orders"))
    .map( e -> Map.entry(e.getKey(), OrderV2.fromV1(e.getValue(), "JBU")))
    .writeTo(Sinks.map("order_v2"));

JobConfig cfg = new JobConfig()
        .setName("bulk-v1-to-v2-jet")
        .setProcessingGuarantee(ProcessingGuarantee.AT_LEAST_ONCE);
...
[source,java]
----
IAtomicLong versionGen = hz.getCPSubsystem().getAtomicLong("orders_v2_version");

Pipeline bulk = Pipeline.create();

// Configure the job
JobConfig bulkJobCfg = new JobConfig();
bulkJobCfg.setName("bulk-v1-to-v2");
...
bulk.readFrom(Sources.<Long, OrderV1>map("orders"))
    .writeTo(Sinks.mapWithEntryProcessor(
        ...
        e -> {
            ...
            OrderV2 v2 = OrderV2.fromV1(e.getValue(), "JBU");
            V2Row newRow = new V2Row(v2, version, "JBU", false);

            // Inline EntryProcessor
            return (EntryProcessor<Long, V2Row, Void>) entry -> {
                V2Row current = entry.getValue();
