...
=== Setting up Heartbeats for WAN Replication

You can specify heartbeats to prevent the connections over WAN to be dropped due to inactivity.
This may be caused by the firewalls blocking the idle connections after some time.
To keep these connections alive, you can set up the following socket options under the advanced network configuration.
    ...
    <advanced-network enabled="true">
        <member-server-socket-endpoint-config>
            <socket-options>
                ...
                <keep-idle-seconds>600</keep-idle-seconds> <2>
                <keep-count>10</keep-count> <3>
