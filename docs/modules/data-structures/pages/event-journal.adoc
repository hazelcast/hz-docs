...
== Configure event journal capacity
...
== Configure event journal time to live
...
The event journal is implemented as a fixed-size ring buffer.
...
some events may be overwritten before they are read.
This is referred to as event loss.

To help you detect and react to event loss, Hazelcast provides lost event
notifications through the `JournalSourceEntry` interface and specific journal sources.
The `JournalSourceEntry` interface includes a method for detecting lost events:

[source,java]
boolean isAfterLostEvents();

This method returns `true` for the first event that follows a lost event in the journal, otherwise `false`.

To obtain these enriched entries, use the journal sources:
[source,java]
