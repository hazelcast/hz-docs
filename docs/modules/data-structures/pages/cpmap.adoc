...
ensure that they are destroyed. See xref:data-structures:distributed-data-structures.adoc#destroying-objects[Destroying Objects]
and xref:data-structures:distributed-data-structures.adoc#cp-data[CP Data Structures].

WARNING: A `CPMap` has a default capacity of 100MB. This capacity can be adjusted for a `CPMap` through
configuration, either programmatically (see: `com.hazelcast.config.cp.CPMapConfig`) or using XML/YAML 
