...
NOTE: Clients attempting to connect to a Hazelcast cluster deployed on OpenShift using the Helm chart and NodePort, may encounter connectivity issues when accessing the cluster outside Kubernetes. For more information about how to resolve this, see xref:kubernetes:troubleshooting-and-limitations.adoc[Troubleshooting and limitations].
