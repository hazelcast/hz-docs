...
|If set to `true` and the table has only one column (beside identifier column), the value of this column will be used as the value in the IMap, instead of `GenericRecord` with only one field.
...
|If set to `true`, all keys will be eagerly read during `MapLoader` initialization.
