...
NOTE: If members have backup reads enabled, some in-memory data may appear to have been kept. However, eventually the backups will be xref:architecture:data-partitioning.adoc#best-effort-consistency[synchronized with the primary partition] (member B).
