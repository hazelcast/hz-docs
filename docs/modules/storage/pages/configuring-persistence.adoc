...
:description: You can configure clusters to persist data structures and job snapshots on disk as well as fine-tune many other options such as backups, encryption, and auto-recovery.
...
Even after enabling Persistence, your members won't start persisting any data until you configure <<data-structures, data structures>>, or <<job, job snapshots>> to be persisted.
...
partial-start automatically on the members that have either the most recent or most complete partition table and that loaded their persisted data within the <<persistence-data-load-timeout-seconds, data load timeout>>. To trigger a partial-start manually, see xref:triggering-partial-start.adoc[].
...
NOTE: If members have backup reads enabled, some in-memory data may appear to have been kept. However, eventually the backups will be xref:architecture:replication-algorithm.adoc#best-effort-consistency[synchronized with the primary partition] (member B).
...
|0
|See <<delaying-migrations, Delaying Migrations>>.
...
NOTE: Sensitive configuration values such as passwords should be protected using xref:configuration:configuring-declaratively.adoc[variable replacers].
...
NOTE: Sensitive configuration properties such as `token` should be protected using xref:configuration:configuring-declaratively.adoc[variable replacers].
