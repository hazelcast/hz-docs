...
In this topology the web application uses Hazelcast clients (TODO link) to connect to Hazelcast cluster on a separate JVM, potentially completely separate machines. Lite members also can be used to on the webapp side.

image:HzSS-Client-Server.png[Hazelcast Spring Session with Client-Server topology, 50%]
