...
* Severity 1 and 2 are only for current outages in production environments.
** Severity 2 is for when a workaround exists (e.g. directing users to static content). Severity 1 is for when no workaround exists.
* Severity 3 is for issues in development or low-impact production issues.
