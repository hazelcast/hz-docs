...
```bash
gradle build
<path_to_jet>/bin/hazelcast submit build/libs/kafka-tutorial-1.0-SNAPSHOT.jar
```
--
Maven:: 
+ 
...
```bash
mvn package
<path_to_jet>/bin/hazelcast submit target/kafka-tutorial-1.0-SNAPSHOT.jar
```
--
====
...
```bash
<path_to_jet>/bin/hazelcast cancel kafka-traffic-monitor
```
