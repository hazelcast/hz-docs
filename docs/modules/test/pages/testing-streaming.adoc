...
- Use `Assertions.assertCollected(...)` or `Assertions.assertCollectedEventually(...)`
...
Pipeline p = Pipeline.create();
p.readFrom(TestSources.items(1, 2, 3))
 .apply(Assertions.assertAnyOrder("unexpected", List.of(1, 2, 3)))
...
Assert.assertEquals("Alice", result.get(0));
Assert.assertEquals("Bob", result.get(1));
----
...
Performs a custom assertion on the full output set. This is a flexible primitive for batch jobs.
