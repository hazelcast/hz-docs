...
NOTE: For security reasons, xref:configuring-declaratively.adoc#variables[configuration variables] and templating are disabled by default in the {enterprise-product-name} REST API. You can enable them by starting members with the system property `-Dhazelcast.rest.allow.config.templating=true`.
