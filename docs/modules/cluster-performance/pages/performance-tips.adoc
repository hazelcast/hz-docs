...
* Since Java is getting better and better at garbage collection, use the latest LTS version
* Use High-Density Memory Store and a small heap
* Applications that do a lot of querying or data updates need more headroom
* A basic tuning brings a huge benefit whereas more tuning may bring almost nothing else except complexity
* Tuning should be reviewed periodically
...
* Security can then be added prior to go-live without rework
...
* You can’t stop a network from a psychical or logical break
...
* CP groups should always be in odd numbers
