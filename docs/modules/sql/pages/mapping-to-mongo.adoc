...
Object type `Collection` resolves columns to the names and types properties of a Mongo collection.
For `ChangeStream` it is more complicated. The ChangeStream objects contains the MongoDB collection properties, prefixed with `fullDocument.`.
...
- `ts TIMESTAMP`: Timestamp of the event. This is either equal to wall time of the event, if provided, or to current time on the Hazelcast member. 
