...
For more information on topologies, see xref:ROOT:topologies.adoc[].
...
For more information on caching in Spring applications, see the following:
...
To build your own database cache, use the Java xref:data-structures:map.adoc#loading-and-storing-persistent-data[MapLoader/MapStore API].
