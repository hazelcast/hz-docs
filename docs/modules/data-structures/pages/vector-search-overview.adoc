...
1. In Hazelcast 5.5 for searches with small `topK` (e.g. 10) it may be beneficial to artificially increase `topK`, adjust `partitionLimit` accordingly and discard extra results. For initial `topK=10` a good starting point could be `topK` increased to 100 and `partitionLimit` between 50 and 100. While this will make the search slower, it will also improve quality, sometimes significantly. Overall this setup can be more efficient than increasing index build parameters (`max-degree`, `ef-construction`) which results in slower index build and searches. With very small `topK` or `paritionLimit` the search algorithm is less able to escape local minima and find best results.
2. Vector deduplication does not incur significant overhead for uploads (usually less than 1%) and searches. You may consider disabling it to get slightly better performance and smaller memory usage if your dataset does not contain duplicated vectors. However, beware that in the presence of many duplicated vectors with deduplication disabled, a  similarity search may return poor quality results.
