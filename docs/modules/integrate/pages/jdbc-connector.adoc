...
You can also use a configured xref:data-structures:working-with-external-data.adoc#defining-external-data-stores[External Data Stores] as a JDBC Source:
```java
Pipeline p = Pipeline.create();
p.readFrom(Sources.jdbc(
...
You can also use a configured xref:data-structures:working-with-external-data.adoc#defining-external-data-stores[External Data Stores] as a JDBC Sink:
