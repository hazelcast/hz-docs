...
NOTE: Regarding metadata space of High-Density Data Store,
Hazelcast automatically manages this memory space. Metadata
space is used by internal data structures of map/cache and
persistence-store. By default `metadata-space-percentage` is set
12.5% of available off-heap space. This value is only used to log
warning messages when metadata space goes beyond it. You can set the
`metadata-space-percentage` property to an appropriate value for your
use case and Hazelcast will log warnings when the value is reached.
