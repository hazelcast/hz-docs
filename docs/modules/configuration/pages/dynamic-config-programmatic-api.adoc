...
You can use the Java member or client APIs to add xref:dynamic-config.adoc[dynamic configuration] for some xref:dynamic-config.adoc#supported-dynamic-configuration-changes[supported features] at runtime.
...
To add new configuration to a member at runtime, use the `add*Config()` method of a xref:dynamic-config.adoc#supported-data-structures[supported data structure] after starting the Hazelcast instance.
