...
For information on how to enable dynamic diagnostic logging and the auto turn-off feature, see the link:https://docs.hazelcast.com/operator/latest/configure-diagnostic-logging[Platform Operator] or xref:{page-latest-supported-mc}@management-center:clusters:diagnostic-logging.adoc[Management Center] documentation.
