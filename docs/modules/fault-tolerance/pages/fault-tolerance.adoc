...
If a job restarts due to a failure before completing its first snapshot, this initial state may be reinitialized.
...
in the xref:pipelines:configuring-jobs.adoc#_job_configuration_options[job configuration].
