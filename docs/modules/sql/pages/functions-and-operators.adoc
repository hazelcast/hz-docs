...
|<<json_query,JSON_QUERY(jsonArg, jsonPath)>> :: JSON
...
- `jsonPath`: A <<jsonpath, JSONPath>>. This path identifies the data that you want to get from the input.
...
|<<json_value, JSON_VALUE(jsonArg, jsonPath [RETURNING dataType])>> :: VARCHAR
