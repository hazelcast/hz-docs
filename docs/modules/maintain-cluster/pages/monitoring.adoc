...
you system runtime stability. Monitor Hazelcast logs and scan for failure
messages regularly. Any incidences should be investigated and documented so as 
o understand your network better and find ways to provision, configure and manage
...
By default, a Hazelcast member writes logs into the `logs` folder of the
...
```bash
PROMETHEUS_PORT=8080 bin/hazelcast-start
```
...
The `Job` class has a
https://jet-start.sh/javadoc/4.5/com/hazelcast/jet/Job.html#getMetrics--[`getMetrics()`]
method which returns a
https://jet-start.sh/javadoc/4.5/com/hazelcast/jet/core/metrics/JobMetrics.html[JobMetrics]
...
https://jet-start.sh/javadoc/4.5/com/hazelcast/jet/core/metrics/JobMetrics.html[JobMetrics API docs].
...
see xref:management:cluster-utilities.adoc#cluster-member-states[Member States]
...
see xref:management:cluster-utilities.adoc#cluster-states[Cluster States]
...
see xref:management:cluster-utilities.adoc#safety-checking-cluster-members[Safety Checking]
...
You can use the xref:management-center:jmx:jmx.adoc[Clustered JMX] interface to integrate the <<management-center, Hazelcast Management Center>>
