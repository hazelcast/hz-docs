...
+
NOTE: To use Management Center to set the cluster version, you need to upgrade Management Center *before* upgrading the members. See xref:{page-latest-supported-mc}@management-center:getting-started:overview.adoc#hazelcast-cluster-compatibility[Hazelcast Cluster Compatibility] for the list of versions that Management Center can set the cluster version to.
