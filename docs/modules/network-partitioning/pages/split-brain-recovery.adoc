...
both sides ensure that there's no intersection in their member lists. In some cases, recovery from
...
The reason is that the master member of the smaller cluster fails to discover the master member of the bigger cluster, so the split-brain merge process never begins.

Considering a scale-up situation; you add new members after some time since the initial member count in the cluster has become
insufficient. The IP addresses of these new members may not exist in the configurations of
the initial set of members, since the new addresses are not added into the the configurations of the initial members while starting them.
...
For these `GET` and `POST` requests, you need to enable the `CLUSTER_READ` and `CLUSTER_WRITE` endpoint groups, respectively, in the cluster members. See xref:maintain-cluster:rest-api.adoc#enabling-endpoint-groups[Enabling Endpoint Groups].

By using this REST endpoint, you are able to add the address of the bigger cluster's master to the smaller cluster's members. Therefore, the smaller cluster master will find the bigger cluster's master
member and the split-brain merge will be able to proceed.
