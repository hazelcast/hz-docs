...
WARNING: Using the extension jar with the Hazelcast cluster that already has the API can cause class mismatch, and thus failures in migration. Therefore, don't add the extension jar to the classpath if your source cluster is newer than 5.3.7 (or 5.4.0 for CP case).
