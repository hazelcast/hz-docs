...
** `PER_NODE`: Maximum number of map entries in each cluster member. This is the default policy. Note that because the limit is enforced at the partition level, the actual number of entries allowed per member is dynamic and depends on the current cluster size and partition distribution, not a static per-member value.
...
=== Partition-based eviction and LRU edge cases

In Hazelcast, eviction is performed on a per-partition basis. When a map reaches its configured size limit, Hazelcast checks each partition individually to determine if eviction is needed. The LRU (Least Recently Used) policy is then applied within that partition: entries in the partition are sorted by their last access time, and the least recently used entry is evicted.

However, in certain edge cases, this can lead to situations where the newest (not the oldest) entries are evicted. For example, if each new key is assigned to a different partition and is the only entry in that partition, then when eviction is triggered, Hazelcast will evict the only entry in the partition even if it is the newest entry in the entire map. This happens because eviction decisions are made locally within each partition, not globally across the entire map. 
