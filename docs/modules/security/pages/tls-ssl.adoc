...
* `keyMaterialDuration`: Duration expression in ISO 8601 format (string such as `PnDTnHnMn.nS`) used for defining cache timeout for keystore and truststore content.
A positive value (e.g. `PT1H`) says for how long should be the key material cached before it's newly loaded.
A negative value (e.g. `PT-1s`) means the key material will be cached indefinitely.
