...
This endpoint takes a URL encoded string of either YAML or XML. You only need to pass this endpoint the `hazelcast` root followed by the new configuration block that you want to add. All existing configuration settings remain unchanged. For example, to add configuration for a map called `new-map`:
