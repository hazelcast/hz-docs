...
+
WARNING: The {open-source-product-name} edition of Hazelcast does not include security settings. To secure your Hazelcast members against clients misusing jobs, you can only <<disabling-code-uploads, disable code uploads>> or <<disabling-the-jet-engine, the Jet engine>>.
...
In Hazelcast {enterprise-product-name}, you can restrict access to jobs, using the following xref:security:native-client-security.adoc[client permissions]:
