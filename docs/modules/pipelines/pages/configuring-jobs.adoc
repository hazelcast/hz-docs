...
The following is an example of how to associate a namespace with a job using the Java API. The namespace must already be configured
...
NOTE: SQL jobs do not support user defined functions and so definition of a UCN is not supported via SQL.