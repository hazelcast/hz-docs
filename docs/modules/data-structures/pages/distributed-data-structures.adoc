...
We recommend that you only use letters, digits, and the `-`, `_`, and `.` characters in the names of data structures.

Special characters will be accepted, but can cause problems for the REST API or xref:management-center:getting-started:overview.adoc[Management Center].

The {open-source-product-name} and {enterprise-product-name} REST APIs get the data structure name from the request URL. A name containing a forward slash (`/`) will cause a `400 Bad Request` error when using the xref:maintain-cluster:enterprise-rest-api.adoc[{enterprise-product-name} REST API], but can fail silently or return the wrong data structure when using the xref:maintain-cluster:rest-api.adoc[{open-source-product-name} REST API].

Management Center uses data structure names in its request URLs. Special characters can prevent it from displaying the configuration of a data structure. See xref:management-center:troubleshoot:management-center.adoc[Management Center troubleshooting].

Asterisks (`*`) in data structure names are especially problematic: configurations for these will treat the asterisk as a wildcard, and could match multiple data structures.

Data structures can't be renamed after creation.
