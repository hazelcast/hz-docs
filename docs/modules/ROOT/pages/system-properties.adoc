...
you need to xref:maintain-cluster:restart-cluster.adoc[restart] the members for which the property is modified.
...
|
|string
|Used to override the cluster version to use while a Hazelcast instance is not yet part of a cluster. The cluster version assumed before joining
...
| 1.0f
| float
| The cleanup threshold for the event journal, expressed as a fraction of its capacity (a value between 0.0f and 1.0f).
When the remaining capacity falls below this threshold, a cleanup may be triggered during put operations to remove expired events (only if a TTL is configured).
A value of 0.0f means cleanup occurs when the journal is full, while a value of 1.0f means cleanup is triggered after each put operation.
Applies to event journals of both IMap and ICache.
...
| `hazelcast.rest.allow.config.templating`
| false
| bool
| Enables xref:configuration:configuring-declaratively.adoc#variables[configuration variables or templating] for updating configuration over the {enterprise-product-name} xref:maintain-cluster:enterprise-rest-api.adoc[REST API].
