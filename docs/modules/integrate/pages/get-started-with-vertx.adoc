...
We will fix that by using a Hazelcast Cluster Manager. This resides in the `io.vertx:vertx-hazelcast`, which is maintained by the Vert.x team with contributions from Hazelcast and is based on Hazelcast {open-source-product-name}.
