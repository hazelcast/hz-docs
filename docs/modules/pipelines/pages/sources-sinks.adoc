...
`?`). For example, if a directory contains log files, named using
...
`lib` directory, or available as a dependency:
...
module to the `lib` directory and the following dependency to your
application:
...
either on the classpath (by putting them into the `lib` directory) of the
...
The map can also be used as a streaming data source by utilizing its xref:data-structures:event-journal.adoc[event journal]. The journal for a map is by default not enabled,
but can be explicitly enabled with a configuration option:
...
only step required is to add the driver to the `lib` directory of Hazelcast or
...
* link:https://docs.hazelcast.org/docs/{page-component-version}/javadoc/com/hazelcast/jet/cdc/DebeziumCdcSources.html[DebeziumCdcSources]:
  generic source for all databases supported by Debezium
* link:https://docs.hazelcast.org/docs/{page-component-version}/javadoc/com/hazelcast/jet/cdc/mysql/MySqlCdcSources.html[MySqlCdcSources]:
...
* link:https://docs.hazelcast.org/docs/{page-component-version}/javadoc/com/hazelcast/jet/cdc/postgres/PostgresCdcSources.html[PostgresCdcSources]:
...
`lib` directory and add the following dependency to your application:
