...
By default, Hazelcast creates more
...
https://jet-start.sh[Hazelcast^]
distributed computing platform as its successors and replacements.

See the following table for the before(MapReduce)/after(Hazelcast)
word count sample.

[cols="1a,1a"]
|===

| *_Before IMDG 4.0 (MapReduce)_* | *_After IMDG 4.0 (Hazelcast)_*
...
By introducing the xref:maintain-cluster:monitoring.adoc#metrics[metrics system] in Hazelcast IMDG 4.0,
...
since the data structure metrics are required for the other xref:maintain-cluster:monitoring.adoc#metrics#metric-consumers[Metric Consumers].
...
+
So, the steps listed in the above xref:maintain-cluster:rolling-upgrades.adoc#rolling-upgrade-procedure[] section
