...
=== Backward Compatibility Issue That Requires Attention

`helm upgrade` command does not allow modifications to `spec.volumeClaimTemplates` property in stateful sets that prevents upgrade which can be seen in link:https://github.com/helm/charts/issues/7803[this helm issue]. Because of this issue, versions between 5.10.7 and 5.10.12 are not upgradable. So, we do not recommend you to upgrade to versions between these version. If you are below version 5.10.7 and want to upgrade, we recommend upgrading to 5.10.13. The following steps can be applied to upgrade:
