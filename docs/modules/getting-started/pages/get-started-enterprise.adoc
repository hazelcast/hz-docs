...
Run each of the following Docker commands in a separate terminal, replacing the `<YOUR_LICENSE_KEY>` placeholders with your Hazelcast {enterprise-product-name} license key:
...
// need to update screenshot to use this ^ after confirming docker host approach is ok
...
// resize image
. Click *View Cluster* and go to **Storage > Maps**. You'll see the map you created has data with three entries. If you want to view the details for `mydistributedmap`, enable the *Map Statistics* box.
+
...
* xref:wan:wan-replication-tutorial.adoc[]
