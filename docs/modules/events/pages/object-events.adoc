...
This method is called by Hazelcast when the `ReliableMessageListener` is cancelled. This can happen when the listener is unregistered, or when it has been cancelled due to an exception or shutdown.
