...
Hazelcast Platform has been tested against the following operating systems. It may run on others that are not listed here.
...
|Windows 10, 11footnote:dev[Suitable for the development of applications (or Hazelcast itself),  _not_ for production. Some features are not available or are not supported.]
...
In addition, Hazelcast offers a range of compatibility guarantees to ensure a predictable upgrade path for your deployment.
