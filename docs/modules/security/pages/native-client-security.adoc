...
when it connects to the clusters. To learn more about this client, see xref:management-center:ROOT:connecting-members.adoc[].
...
NOTE: Hazelcast cannot check permissions in code that's uploaded with a job, If you enable the `upload` permission, clients can upload custom code that ignores any configured permissions.
