...
NOTE: Although running multiple members on a single Docker host is useful for testing, it's not suitable for production. To run a physically distributed cluster in containers, see xref:deploy:deploying-with-docker.adoc[] or xref:deploy:deploying-in-kubernetes.adoc[].
...
+
You should see that entries and their backups are distributed evenly across each member. This process is called partitioning. You can learn more about this subject in xref:architecture:data-partitioning.adoc[].
...
- xref:architecture:data-partitioning.adoc[Data partitioning]
