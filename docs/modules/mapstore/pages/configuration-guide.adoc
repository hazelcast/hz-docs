...
:description: This topic explains how to configure a map with a MapStore. It includes details of how to configure the map to connect to a external data store using a MapStore, as well as configuring caching behaviours, and MapStore reuse. 
...
NOTE: Follow the instructions in xref:mapstore:configuring-a-generic-maploader.adoc[] or xref:mapstore:configuring-a-generic-maploader.adoc[] if you are using a pre-built Mapstore.
