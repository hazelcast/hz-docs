...
To use a persistence memory device, you don't need to make any changes to your application code. The following example shows you how to configure the dual in-line memory modules (DIMMs) of Intel(R) Optane(TM) DC as the High-Density Memory Store in Hazelcast.
...
NOTE: Currently the only supported configuration is Intel(R) Optane(TM) DC DIMMs running on Linux x86_64.
...
** `SYSTEM_MEMORY`: If you choose this mode, the memory is onlined as system memory (aka KMEM DAX).
