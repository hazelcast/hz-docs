...
The Java cluster routing mode specifies how the client connects to the cluster. It can currently be used only with Java clients.
...
license is present on your Enterprise cluster, then clients in this routing mode are able to leverage this to send CP operations
...
If the connection lost, which occurs only if all members of the partition group become unavailable, there is no attempt to retry the connection before failing over to another partition group.
...
+
In a scenario where all group members are killed almost simultateously, the client loses connection but reconnects when a member starts again.
...
is present on your Enterprise cluster, then clients in this routing mode are able to leverage this to send CP operations directly
...
=== Configuring Client Cluster Name
...
this functionality allows clients to receive a mapping of CP group leadership from the cluster and use it to send
...
direct to leader routing for CP operations, clients will be able to send all operations directly to their group leaders,
...
instead of using a faster internal cluster link and routing through another member. It should also be considered that
direct to leader routing can put uneven pressure on the cluster if CP group leaders receive substantially more load than
other members of the cluster - this is particularly problematic when only 1 CP group leader is present.
...
The following is the declarative configuration equivalent of the above example:
