...
----
+
Please note, that this call should be done before instance creation (before the call to `Hazelcast.newHazelcastInstance`).
. Remove index configuration for `PRINCIPAL_ATTRIBUTE_NAME` **if** no other IMap customization is wanted; it will be configured automatically. If you want to further customize session IMap, you can:
- create whole `MapConfig` manually and then create an index of type `HASH` on `principalName` attribute
- or you can create a bean of type `SessionMapCustomizer` to apply your customizations on predefined `MapConfig`.

=== List of breaking changes

4.0.0 is a major release and it contains some breaking changes:

- Group ID and artifact ID were changed to `com.hazelcast.spring` / `hazelcast-spring-session`.
- Main package was changed from `org.springframework.session.hazelcast` to `com.hazelcast.spring.session`.
- Serialization was reworked completely; it no longer uses an optional `HazelcastSessionSerializer`, but relies on Compact Serialization with two new serializers: `com.hazelcast.spring.session.AttributeValueCompactSerializer` and `com.hazelcast.spring.session.HazelcastSessionCompactSerializer`. Additionally serializer of `SessionUpdateEntryProcessor` will be configured automatically on server nodes if jar is deployed on server nodes.
- The binary representation of the data was changed; Instead of using Spring's `MapSession`, it uses `com.hazelcast.spring.session.BackingMapSession` with attributes stored as `com.hazelcast.spring.session.AttributeValue`. This change was done to enable true client-server deployments, without the need for any custom code deployment on server nodes for classes used as session attributes.
- Method `void setDefaultMaxInactiveInterval(Integer defaultMaxInactiveInterval)` in `HazelcastIndexedSessionRepository` was removed; please use an alternative with `Duration` as a parameter.
- Most of the setter methods in `HazelcastIndexedSessionRepository` return now `HazelcastIndexedSessionRepository` (`this`) for fluent API.*
- The default logging framework was changed to SLF4J; the default annotations library is now JSpecify.
- Project is now fully supporting Java Modules
