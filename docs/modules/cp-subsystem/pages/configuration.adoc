...
When CP Subsystem persistence is enabled, all Hazelcast cluster members -- including AP members -- create a directory under the base `cp-data` directory. CP members persist CP state in these sub-directories. AP members persist only their status as non-CP members. Ensure that you do not delete these directories.
...
Hazelcast also offers xref:storage:persistence.adoc[persistence] for some AP data structures. If you persist AP and CP data structures in a single Hazelcast cluster, be aware that member or cluster restarts can fail if the persisted state (CP or AP) becomes stale. You can configure AP and CP members to automatically remove stale data with the `auto-remove-stale-data` option -- see xref:storage:configuring-persistence.adoc[Configuring persistence]. Otherwise, you will need to remove stale data manually.
