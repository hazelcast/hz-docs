...
To update this configuration, complete the following steps:
...
* Migration report
+
This is written to the _migration$$_$$report$$_$$[migration_id].txt_ file in the _bin_ folder of the extracted DMT package.
+
The logs are in the format `[(migration_id)_(member uuid)] (member log)`

* DMT log file
+
This is the file specified in the `--log.path` flag of the `start` command. The file is saved to the _~/.hazelcast_ folder.
+
NOTE: Logging uses the same environment variables as Hazelcast CLC. For further information on environment variables, refer to the https://docs.hazelcast.com/clc/latest/environment-variables[Environment Variables^] section of the Hazelcsat CLC documentation.

* `__datamigration_results` IMap
+
This is created on the target cluster.
+
The keys are uuid4 string format migration IDs, and the values are `HazelcastJsonValue`s that correspond to migration statuses.
+
