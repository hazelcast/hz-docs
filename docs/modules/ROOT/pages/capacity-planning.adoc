...
Hazelcast supports the **last two Java LTS versions** available at the time of its release. Newer JDK releases often contain performance improvements, consider upgrading to 
xref:deploy:versioning-compatibility.adoc#supported-java-virtual-machines[a newer supported version].
The garbage collectors of the newer versions have been improved to achieve much higher throughputs. 
